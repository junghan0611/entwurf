...
| Gemini backend's MCP function-schema asymmetry is an observed Gemini ACP surface asymmetry, not a closable overlay leak | **L1** | BASELINE.md 2026-05-03 — model self-reported MCP servers connected but absent from native function schema, and described MCP / custom-tool access as shell-mediated (`run_shell_command`) rather than direct function calls. Same shape would need to reproduce against a non-bridged direct gemini --acp run to firm the claim that this is upstream surface, not bridge-induced. | Echo chamber — only the bridged ACP path was tested | L2/L3: spawn `gemini --acp` directly without the bridge MCP injection and observe whether MCP servers appear in function schema there |
