...
| Gemini backend operator-config isolation overlay closes 5 channels (0.4.8): native system body, operator memory paths, native tool surface, `GEMINI.md` hierarchical discovery, MCP whitelist | **L2 + L3** | BASELINE.md 2026-05-03 entry — model classified canary string under "actual system prompt", invoked all 4 read-class tools without `denied by admin policy`, reported overlay-only memory paths, no `GEMINI.md` awareness, only the 2 bridge MCP servers. Plus on-disk: `check-backends` synthetic test (124 assertions covering settings/admin.toml/system.md/symlinks/empty-dirs) + `~/.pi/agent/gemini-config-overlay/` shape on disk. | Single backend / single model run; no fault injection; subscription-billing parity unverified | L4: same baseline run with operator's GEMINI_API_KEY explicitly set vs unset to confirm oauth-personal isolation; L5: 50+ turns under the gemini backend with periodic canary recheck |
| Gemini backend's MCP function-schema asymmetry is a Gemini ACP property, not a closable leak | **L1** | BASELINE.md 2026-05-03 — model self-reported MCP servers connected but absent from native function schema, routes via `run_shell_command`. Same shape would need to reproduce against a non-bridged direct gemini --acp run to firm the claim that this is upstream surface, not bridge-induced. | Echo chamber — only the bridged ACP path was tested | L2/L3: spawn `gemini --acp` directly without the bridge MCP injection and observe whether MCP servers appear in function schema there |
