...
| Skill + MCP capability dignity is preserved across Claude / Codex / Gemini | **L3 / L4 mixed** | `check-bridge` covers visibility/invocation across available backends, and BASELINE history records the earlier Gemini asymmetry retraction. The current invariant is parity of capability, not identical tool spelling or identical backend implementation. | Long-session stability of the advertised surface is not yet soaked. | L5: repeat MCP/skill callable checks during long sessions and after backend context-pressure events. |
