...
| macOS Entwurf-only install | `macos-install-surface` CI job (candidate, non-voting) | NOT YET MEASURED. A green run there would certify the Entwurf-only install path only — never Claude/Copilot/OMP/agy wiring, the pi rail, ACP or mux. `continue-on-error` keeps it off the release conclusion until GLG promotes a first physical green. That rests on a vendor-documented semantic (docs.github.com workflow-syntax, `jobs.<job_id>.continue-on-error`: *"Prevents a workflow run from failing when a job fails."*, fetched 2026-09-09) with no precedent in this repo (measured: main 0). It fails closed — a residual edge makes the run non-success, the oracle ABORTs, and the cut stops, so what is staked is availability, never evidence-truth — and it measures itself: the assumption bears load only when a job here is RED, which is the same run whose conclusion observes the semantic directly. The first red job settles it in one push. |
