...
Note: pi-shell-acp does not implement compaction (0.5.0 declaration). The provider registers a `session_before_compact` handler that cancels every pi-side compaction trigger (silent overflow, threshold, explicit-error overflow, and manual `/compact`) — pi-side summary does not reduce the backend transcript, so leaving it on would be a quiet lie. Backend-native auto-compaction is **allowed** by default (0.5.0); Claude/Codex compact themselves and the pi session survives that. Knobs: `PI_SHELL_ACP_ALLOW_PI_COMPACTION=1` opts back into pi-side compact (rare); `PI_SHELL_ACP_DISABLE_BACKEND_COMPACTION=1` restores the 0.4.x guards (Claude `DISABLE_AUTO_COMPACT=1` + `DISABLE_COMPACT=1`, Codex `-c model_auto_compact_token_limit=i64::MAX`) as an escape hatch for a misbehaving backend. Legacy single knob `PI_SHELL_ACP_ALLOW_COMPACTION=1` is rejected at spawn intent with a next-action message naming both new knobs. For long sessions, the footer percentage shows the backend's own `usage_update.used / size` value, the same signal peer ACP clients (zed, obsidian-agent-client, openclaw-acpx) display. Both supported backends emit per-turn occupancy: claude-agent-acp via `input + output + cache_read + cache_creation` of the last assistant result, codex-acp via `tokens_in_context_window()`. The `[pi-shell-acp:usage] meter=acpUsageUpdate|componentSum source=backend|promptResponse backend=… used=… size=… raw: input=… output=… cacheRead=… cacheWrite=…` diagnostic line carries the per-component breakdown and the meter mode for audit. Near limit, choose a visible action: `/clear`, switch model, or let the backend auto-compact on its own. Verification: `./run.sh smoke-compaction-policy` (4 deterministic steps; `LIVE=1` adds 3-prompt survives-compact probe steps 03/04 against Claude + Codex — real ACP spawn, literal `/compact` as backend prompt, sentinel recall on the other side). Gemini is out of scope at 0.5.0.
...
| 0.5.0 compaction policy: bridge does not implement compaction. Backend-native compaction allowed by default; pi-side blocked; legacy single knob throws at spawn intent; ACP backend (Claude + Codex) survives a backend `/compact` with the bridge alive | **L3 deterministic + L4 live (dual-classifier)** | `./run.sh smoke-compaction-policy` 4-step deterministic gate (no guard env / argv injected by default; pi-block message names the boundary and points at the backend-native path; legacy `PI_SHELL_ACP_ALLOW_COMPACTION=1` throws on both wrapper `resolveAcpBackendLaunch` AND production `createBridgeProcess` paths naming both new knobs; `PI_SHELL_ACP_DISABLE_BACKEND_COMPACTION=1` escape hatch restores 0.4.x guards) PLUS `LIVE=1` adds steps 03/04 — real ACP spawn via `runEntwurfSync`, plant sentinel, send literal `/compact` as backend prompt, recall sentinel. Dual-classifier evidence path: text classifier ("compacted"/"summarized"/"context reduced" in (b)-turn reply) OR wire classifier (bridge stderr `[pi-shell-acp:usage]` shows explicit `used=0` compact_boundary, or >=50% used drop). codex-acp signals on text ("Context compacted"); claude-agent-acp signals on wire (`used=0` synthetic usage_update per acp-agent.js:477-498). Both backends pass under this dual path with the bridge persisted-mapping reused across all three turns | Gemini out of scope at 0.5.0; live steps require operator-authenticated Claude/Codex and cost a few cents per backend, so they do not run as part of pre-commit `pnpm check` | L5: 50-turn run per backend with periodic `/compact` and sentinel recall under varying compaction triggers; automate Gemini `/compact` semantics check |
