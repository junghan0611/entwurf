...
| Native Codex host | first admission accepted 2026-09-12 under the fixed-home topology #95 D1 later retired; the caller-seat re-run is PENDING | Operator-owned birth/MCP/statusline atoms and the on-demand four-coordinate chain were measured on Linux at Codex 0.153.4: initial Pi outside the then-required exact `codex` home; app-server, omitted-placement fresh Codex, and outbound Pi inside it; exact callbacks and addressed delivery both ways. That chain is PAST CONTRACT — the current one puts the app-server in its own session A and the Pi/Codex/outbound-Pi trio in a different session S, and no LIVE receipt for it exists yet. `DELIVERY.md` owns the 57-assertion receipt and digest. Arbitrary attached-TUI placement is unsupported and unclaimed. |
