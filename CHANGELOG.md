...
- **`check-pack-install` no longer leaks into the operator's real XDG roots — and proves it.** The gate swapped `HOME` per drive but inherited the operator's exported XDG roots, so its `run.sh install` drive wrote a **foreign pi-provider install-state into the real `~/.local/share/entwurf`** and the agy-imprint drive appended fake birth lines to the real `~/.local/state/entwurf/agy-imprint.log` — the same class as the 2026-07-13 hard-verify pollution, one layer deeper: inside `run.sh` itself, where S5/S5b (which scan only `scripts/*.sh`) cannot see. Every sandbox drive now exports `XDG_DATA_HOME`/`XDG_STATE_HOME`/`XDG_CACHE_HOME` alongside `HOME` (the operator-command drives also gain hermeticity: `doctor-pi-provider` no longer reads the real host's install-state), and a **self-fence** snapshots the operator's real install-state tree before the gate and fails loud if a single byte moved. Mutation-checked: dropping one drive's `XDG_DATA_HOME` swap makes the fence fail and name the leaked file. The live wiring itself (agy mcp_config, hooks, statusLine, dev-bin links, pi settings) was never touched by the gate — the leak was state/log-grade, not install-grade.
