...
- **Snowflake Cortex Code joins the ACP rail as the first non-claude backend.** `backend: "cortex"` lands as one `cortexAdapter` object in `pi-extensions/lib/acp/backend-adapter.ts`, registered in `ADAPTERS = [claudeAdapter, cortexAdapter]`; the common turn loop is untouched. The adapter resolves `cortex acp serve` from PATH (the Cortex CLI is itself the ACP server — no `*-acp` package), appending `-c <connection>` / `-m <native-model>`; the curated `cortex-auto` id emits no `-m` so Cortex picks its own default. A `CORTEX_ACP_COMMAND` override runs via `bash -lc` with the selection flags shell-quoted and appended after the operator string, so the bridge's model/connection choice wins (later yargs args override earlier ones). The curated surface (`models.ts`) is `cortex-auto`, `cortex-claude-opus-4-6`, `cortex-claude-haiku-4-5`, `cortex-claude-sonnet-4-6`, and `cortex-openai-gpt-5.2`, each carrying the reserved `cortex-` prefix that is the single routing authority — `routeModel` strips the prefix to recover the native id. Cortex's only own setting is the Snowflake connection name (`entwurfProvider.cortexConnection`, overridable per-shell by `ENTWURF_ACP_CORTEX_CONNECTION`), which participates in the session config signature so a connection change invalidates a reused session. The Cortex config overlay (`overlay.ts`) redirects `SNOWFLAKE_HOME` to a pi-owned `~/.pi/agent/cortex-config-overlay` and symlinks only the operator's existing auth/config (`connections.toml`, `config.toml`, `cortex/cache`, `cortex/skills`) through — it never copies, parses, or mediates Snowflake credentials (Hard Rule #8) — while sweeping `conversations`/`memory`/`profiles`/`logs` each spawn and stripping any stale operator-state symlink off the allowlist. Cortex is carrier-less: it exposes no `_meta.systemPrompt`, so operator engraving rides the first-user augment (`augment.ts`) instead of a system-prompt carrier. Coverage: the deterministic `check-acp-cortex` gate (in `pnpm check`) asserts curated-model registration, `cortex-` prefix routing → `cortexAdapter`, prefix-strip → native `-m`, `cortex-auto` → no `-m`, overlay auth-through, and `CORTEX_ACP_COMMAND` override quoting. An on-demand live turn is exposed as the `smoke-acp-cortex-live` run.sh target (honest-skip unless `LIVE=1` and `cortex` is on PATH with a prior `cortex auth login`); its live-turn script `scripts/smoke-acp-cortex-live.ts` is a pending deliverable, and the target fails loud until it lands. Cortex is a deterministic-gated + on-demand-live surface and is deliberately **not** in the claude-only live release floor (capability dignity, invariant #7).
