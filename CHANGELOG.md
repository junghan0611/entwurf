...
## Unreleased — v2 core + ACP plugin on v2 substrate (2026-06-19, not yet tagged)

> This release is the **v2 dispatch substrate (`entwurf-core`) + meta-bridge with an ACP plugin re-added on top**. The v1 entwurf verbs are gone and `entwurf_v2` is the spine; the ACP plugin (Claude-first) is a fresh build on the v2 core — it enters as a pi provider/model on a host `--entwurf-control` session that is *already* a v2 socket-citizen, and never mints its own socket / peers / citizen layer (the old fat `acp-bridge.ts` served only as a behavior oracle). The package name `pi-shell-acp` is kept on purpose — ACP lives, so the name is honest, and there is no rename. Verified green throughout: `pnpm check` EXIT 0, and the release-gate MUST tier carries the ACP plugin acceptance floor (7 LIVE smokes, S1~S2f) alongside the v2-native live gates.

### Added

- **ACP plugin on the v2 core.** Provider registration via `pi-extensions/acp-provider.ts` (thin entry) + `pi-extensions/lib/acp/*`, a curated Claude model registry (`claude-sonnet-4-6` + `claude-opus-4-8`), a lowercase-hyphen no-auth sentinel (no credential proxy/resale/bypass), and a loader/fence with `check-auth-boundary` + `check-acp-provider-surface`.
- **Claude ACP backend.** Pinned deps (`@agentclientprotocol/claude-agent-acp@0.39.0`, `@agentclientprotocol/sdk@0.22.1`, peer-resolution pin `@anthropic-ai/sdk@0.100.1`), a raw stdio JSON-RPC turn over the package bin, a `CLAUDE_CONFIG_DIR` config overlay (isolation + tool-narrowing + explicit `hooks: {}` = mailbox-absent-by-design), a pure `assertExcludeToolsHonored` tool-surface preflight, ACP→pi event mapping, and the real `streamSimple` backend replacing the fail-loud stub.
- **Session reuse + carrier + augment.** Process-scoped in-memory ACP session reuse with a `bootstrapPath`-scoped prompt builder (new = full transcript / reuse = latest-user delta only), a short and pure billing/memory carrier (engraving, tiny non-empty default for Claude preset replacement; rich context never rides it), and a first-user context augment that prepends rich operator context to the wire prompt (not the carrier).
- **Always-on turn-progress visibility.** Stream lifecycle notices (`[acp: preparing claude session]` → `… session ready` → `… sending prompt`, or the reuse variant) carried by a display-only `pi-shell-acp:lifecycle-notice-v1` marker so they never replay into rebuilt transcripts or perturb session-reuse signatures.
- **Acceptance gates.** Deterministic `check-acp-*` gates (overlay / tool-surface / event-mapper / prompt-builder / session-store / session-reuse / carrier-augment / sdk-surface / backend-preflight) inside `pnpm check`, plus 7 ACP LIVE smokes (socket-citizen / raw-turn / overlay / provider / session-reuse / carrier-augment / rgg) added to the release-gate MUST tier as LIVE-gated honest-SKIP steps.

### Changed

- **ACP is a plugin, not the boundary.** The host `--entwurf-control` pi session supplies socket citizenship; the plugin owns only the backend process lifecycle, the config overlay, the per-backend ACP dialect, and backend/turn evidence — never socket/peers/citizen registration or liveness facts.
- **Billing carrier discipline (핀1).** The Claude `_meta.systemPrompt` carrier is kept short and pure (clock/random/env-free); rich context rides the first user message. A carrier that drifts from the SDK-default shape can route subscription billing to metered classification (HTTP 400 for credit-less subscribers) — so it is held small by construction.
- **env sender seam + `entwurf_peers` two-rail.** `updateSessionEnv` sets `PI_AGENT_ID=<provider>/<model>` alongside `PI_SESSION_ID`; `entwurf_peers` routes both the control-socket and meta-record rails through one `listEntwurfFacts` fence (bounded output, latest 32 entries).
- **Docs realigned to "v2 core + ACP plugin".** `AGENTS.md`, `README.md`, and `ROADMAP.md` rewritten — ACP is a returning plugin on the v2 spine, Claude-first, with the v1 surface documented as gone.
- **pi runtime bumped 0.79.8 → 0.80.2.** pi 0.80 moved the standalone root `getModels()` to the deprecated `@earendil-works/pi-ai/compat` entrypoint (the global-API churn). The curated Claude surface (`pi-extensions/lib/acp/models.ts`) imports `getModels` from `/compat` — NOT the 0.80 provider-factory subpath (`providers/anthropic`): although that subpath typechecks against `node_modules`, it is NOT resolvable under pi's extension loader, whose jiti alias map (pi-coding-agent `core/extensions/loader.ts`) maps only the bare root, `/compat`, and `/oauth` for extensions (all → `ai/dist/compat.js`); a `providers/*` import prefix-matches the bare alias and resolves to the unresolvable `dist/compat.js/providers/anthropic` (caught live by `smoke-resident-garden-guard`'s extension-load crash, invisible to static typecheck). Reading the static catalog is env/credential/network-free, preserving the #26 auth-boundary invariant (the curated surface consumes no key). `check-pi-import-surface` allowlists exactly ONE subpath, `@earendil-works/pi-ai/compat` (closing-quote-anchored; every `/providers/*` and `/oauth` stay forbidden). Pins updated across `package.json` (devDep `0.80.2` + peer `>=0.80.2 <0.81`), `check-pi-runtime-version` floor, `check-pack-install` peers, and `AGENTS.md`. All other pi-ai/pi-coding-agent symbols verified surviving on the 0.80.2 root. This `/compat` use is an extension-loader compatibility shim forced by the loader's alias map, not a casual adoption of a deprecated API; the `<0.81` ceiling pins it so 0.81 (which may remove `compat` or change the loader alias) triggers a re-evaluation against the then-current root/loader surface.
- **Claude ACP adapter bumped 0.39.0 → 0.50.0; ACP wire SDK 0.22.1 → 0.29.0.** 0.50/0.29 removed `unstable_setSessionModel`; model enforcement moved to `setSessionConfigOption({ sessionId, configId: "model", value })` — the adapter resolves a full id or alias to a canonical model id and routes it through `query.setModel`. Migrated the production backend (`pi-extensions/lib/acp/backend.ts`), the model-enforcing LIVE smokes (raw-turn / overlay / memory-containment), and the session-reuse fake seam. `@anthropic-ai/sdk` stays at `0.100.1` (still satisfies `@anthropic-ai/claude-agent-sdk` 0.3.186's `>=0.93.0` peer); `check-acp-sdk-surface` pins/lock-regex/runtime-probe updated to 0.50.0 / 0.29.0 / 0.3.186. On 0.29.0 the now-`@deprecated` `ClientSideConnection` was migrated off (this is the previously-deferred builder migration): a single `connectAcpClient` adapter (`pi-extensions/lib/acp/acp-client.ts`) wraps the fluent `client().connect(stream)` — `connect()` (persistent), NOT the op-scoped `connectWith()`, because the backend retains and reuses the connection across turns — and drives the agent via `request(AGENT_METHODS.*)`. The `AcpConnectionLike` seam + backend orchestration + the `check-acp-session-reuse` fake-seam gate are preserved; the production backend and the three model-enforcing LIVE smokes (raw-turn / overlay / memory-containment) share the one adapter. Teardown now explicitly closes the SDK connection before child process cleanup on non-retained/error paths; this is load-bearing for the fluent SDK handle (raw LIVE proved a post-PASS hang without it). `check-acp-sdk-surface` now guards the value exports the real code uses (`client`, `ndJsonStream`, `PROTOCOL_VERSION`, `AGENT_METHODS`, `CLIENT_METHODS`).

### Removed

- **The legacy fat ACP bridge**, replaced by the plugin build above: the root ACP sources (`index` / `acp-bridge` / `event-mapper` / `engraving` / `pi-context-augment`) are gone in favor of `pi-extensions/acp-provider.ts` + `pi-extensions/lib/acp/*` (a fresh build on the v2 core, not a port of the old architecture).
- **v1 entwurf entrypoints.** MCP tools `entwurf` / `entwurf_resume` / `entwurf_send`, the pi-native `entwurf_send`, the `spawn_async_resume` control RPC, and the `/entwurf` / `/entwurf-send` / `/entwurf-status` commands. The surface is now `entwurf_v2` + `entwurf_peers` + `entwurf_self` + `entwurf_inbox_read`, with `/entwurf-sessions` and `/gnew` operator commands.
- **OpenClaw plugin importer.** Deprecated and dropped from the install surface.

### Deferred

- **Persisted resume/load** — current reuse is in-memory + record write only; persisted read/use stays off.
- **Non-Claude ACP backends / Cortex lane** — vendor/governed CLIs once local full verification is possible.
- **Fresh sibling minting** — every v2 transport targets an already-identified citizen; minting a brand-new sibling from nothing is an explicit, documented capability hole (not a silent regression) pending a later `spawn-fresh` lane.
- **Test / release-gate taxonomy** ([#41](https://github.com/junghan0611/pi-shell-acp/issues/41)) — reclassify verification assets into deterministic / MUST-live / BEHAVIOR / utility tiers.
