...
//     FAILS acceptance unless PI_SHELL_ACP_OVERLAY_ALLOW_PATH_FALLBACK=1 (then
...
const REQUESTED_MODEL_ID = process.env.PI_SHELL_ACP_OVERLAY_MODEL ?? "claude-sonnet-4-6";
const ALLOW_PATH_FALLBACK = process.env.PI_SHELL_ACP_OVERLAY_ALLOW_PATH_FALLBACK === "1";
		...
		if (!ALLOW_PATH_FALLBACK) {
			fail(
				`could not resolve @agentclientprotocol/claude-agent-acp package bin (${(err as Error).message}). ` +
					...
					"PI_SHELL_ACP_OVERLAY_ALLOW_PATH_FALLBACK=1 only for debug.",
			);
		}
	}
	return {
