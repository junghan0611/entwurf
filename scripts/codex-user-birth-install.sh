#!/usr/bin/env bash
# codex-user-birth-install — publish the Codex SessionStart birth unit at USER paths.
#
# The USER-layer twin of scripts/codex-birth-install.sh. Same unit, same payload, same
# hooks grammar; the only differences are where it lives and who owns it:
#
...
#
# WHY A USER TWIN EXISTS. The System-layer unit avoids the vendor's interactive hook-trust
# prompt because `/etc/codex` is a MANAGED config layer (`[source]` hooks/src/engine/
# discovery.rs:794-815 — managed handlers skip `trusted_hash` entirely). The price is root,
# and on a user-installed Codex (NixOS, pnpm-global) root is not a thing the operator has
# agreed to spend. A USER declaration is not rejected — it is TRUST-GATED: it runs after the
# operator answers "Trust all" once, and the vendor records that decision itself.
#
...
die() { printf '[codex-user-birth-install] %s\n' "$1" >&2; exit 1; }
note() { printf '[codex-user-birth-install] %s\n' "$1"; }
...
[ "$(uname -s)" = "Linux" ] || die "Linux only, like its System-layer twin."
[ "$(id -u)" != "0" ] || die "refusing to run as root: this unit owns USER paths, and publishing them as root would leave files the operator cannot replace. Use the System-layer unit if root ownership is what you want."
...
# The System twin asks the same three questions of root; here the subject is the operator.
...
#!/usr/bin/env bash
# GENERATED by codex-user-birth-install.sh (entwurf $UNIT_VERSION) — DO NOT EDIT.
...
if [ ! -x "\$NODE_BIN" ]; then
  ...
  echo "entwurf codex-birth: re-run './run.sh install-codex-user-birth' after a node change." >&2
  exit 1
fi
if [ ! -f "\$PAYLOAD" ]; then
...
{
  "description": "entwurf codex-birth $UNIT_VERSION (user layer) — mints one garden meta-record per top-level Codex thread on its first turn. Managed by scripts/codex-user-birth-install.sh; inverse: scripts/codex-user-birth-uninstall.sh. Do not edit: the exact bytes are recorded in $STATE_FILE. The vendor's trust decision for this declaration lives in config.toml and is the operator's alone.",
  "hooks": {
    "SessionStart": [
      {
        "hooks": [
...
write_state() { # $1 = status
  ...
  cat > "$STATE_TMP" <<STATE
{
  "schema": "codex-user-birth-install-state/v1",
  "status": "$1",
...
mkdir -p -- "$HELPER_DIR/lib/native-push"
