...
- **후속 2건 랜딩 (GLG 지시로 진행)**:
  - **기존 호스트 V3 마이그레이션 사전 안내** — main이 GLG의 *모든 개발 PC*를 뜻한다는 게 확인되면서 생긴 공백이다. 다른 PC의 store는 pre-cut(v1/v2)이라 첫 세션이 전 identity 표면에서 fail-loud한다. 반응형 처방은 이미 완비돼 있었지만(M1_PRESCRIPTION이 dev/설치본 양형식 인쇄) **사전 안내가 durable 문서에 없었다** — clean-host 워크스루는 store가 없는 호스트 기준이라 해당이 안 된다. README에 verify→migrate 절차 + backup/restore 대칭 + `--drop-parentage`가 사람의 결정이라는 점을 명시, `docs/setup-clean-host.md`에는 "이 문서는 clean 호스트 가정"이라는 경계와 함께 넣었다. **호스트당 1회.**
  - **실패 시 transcript 자동 보존** — 오늘의 오진이 가능했던 기계적 이유다. `smoke-acp-v2-send-live`는 실패 시 아무 stream도 남기지 않아(tail이 `ENTWURF_SMOKE_VERBOSE` 뒤에 있고 aggregate/CI는 그걸 안 켠다) GPT가 pi 세션 JSONL까지 파야 했다. 이제 두 ACP 스모크 모두 실패 시 **전체 transcript를 world 밖 파일로 쓰고 경로를 인쇄**한다(best-effort — 아티팩트 쓰기가 원래 단언 오류를 가리지 않는다). bundled-mcp의 기존 30줄 tail은 유지.
