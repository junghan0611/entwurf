...
## 검수 차터 — 다음 세션이 빡세게 볼 것 (전제 없이, 오늘 세션의 주장을 재검증하라)

1. **최종 HEAD에서 3셀 + LIVE 재획득.** (c) 증거는 `7cbeb29` 바이트에 묶여 있고 이후 docs 커밋들로 HEAD가 움직였다(당시 판단: docs-only라 재획득 불필요 — 오푸스, 페블 동의. **이 판단 자체를 검증하라**). 주의: `run.sh`는 tarball에 들어가므로 docs 커밋이어도 tgz sha256은 움직인다 — 재획득 시 새 sha를 그 HEAD SHA에 묶어 기록. `LIVE=1 ./run.sh release-gate <mkdir한 fresh-scratch>` 기대값 = **MUST 17/17 + BEHAVIOR 1/1**.
