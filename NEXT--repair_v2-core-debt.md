...
2. **C2 — 단일 경계 cut (② 확정, GLG 2026-07-23)**: C2-lite/C2-full 분할은 폐기한다. 분할하면 gardenId ↔ socket key가 갈라지거나(주소는 record, socket은 native id) gardenize mirror에 기대는 과도기 dual-authority를 커밋하게 된다 — #50 원문대로 **subtraction + record cut이 함께 GREEN인 한 커밋**이 첫 checkpoint다. 구성:
   - **삭제(subtraction)**: `--session-id` 주입(entwurf-resume-args.ts:83, entwurf-core.ts), `setSessionName` garden mirror, `/gnew`/`garden-new`, builtin `/new` block, garden-format hard exit, custom pre-created session header/name 경로.
   - **record 권위**: pi SessionStart → upsert(`backend:"pi"`, nativeSessionId=pi native id). upsert는 attach — `(backend, nativeSessionId)` lookup으로 기존 record에 붙고 없을 때만 mint(재오픈이 새 gardenId를 만들면 버그). gardenId는 record가 민팅한다 — 세션 id와 무관, 동일성은 불변식이 아니다(LOCKED PROTOCOL 1·2).
   - **socket 전환**: control socket 생성과 dispatch resolve를 record gardenId 한 축으로. `smoke-resident-garden-guard` 뒤집기 — 생존 = record mint + record-gardenId socket (uuid.sock 아님).
   - 중간 RED 커밋 없음, 정상 라우팅의 새 dual-authority 없음 — 이 두 줄이 ②를 고른 이유 그 자체다.
3. **pi-attach smoke** (mkdtemp 격리, live store 무접촉) = C2 커밋의 게이트 tail이자 slice 종착점. main의 `check-bridge-delivery` 셀에서 **드라이버만 재사용**(빌드된 dist를 MCP stdio로 몰아 실제 tools/call) — backend 문자열 스왑이 아니다. Claude=mailbox rail, pi=socket rail이므로 pi 셀은 별도 fixture/assertion: SessionStart attach → live socket(record gardenId key) → RPC ack.
