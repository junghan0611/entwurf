...
- **[업그레이드 하네스 확장 랜딩 — GLG 지시 2026-07-24 밤, 구현 오푸스 / 검수 GPT `20260724T185330-e446d2`]** 위 「기존 호스트 V3 마이그레이션 사전 안내」는 **문서만** 있었고, 그건 "main이 모든 개발 PC에 퍼져도 된다"의 근거가 못 된다는 GPT 판정이 나왔다. 3셀 하네스가 그 전환을 증명하도록 확장했다.
  ...
  - **🟡 미완 검증 1건 (다음 세션이 닫을 것) — F8의 in-gate 발화가 재현되지 않았다.** sentinel 자체는 확실히 동작한다: store 게이트를 무조건 통과하도록 변조한 installer를 **격리 실행**하면 sentinel 로그에 `--version`이 남고 installer가 floor 검사에서 rc=1로 죽는다(실측). 그런데 **같은 변조를 게이트 안에서 걸면 C6/C7은 RED가 되는데 F8은 여전히 `✅ … sentinel stayed untouched`**였다. 즉 sentinel은 발화하는데 F8이 그걸 못 읽었거나, 게이트 내 드라이브가 다른 지점에서 죽는다. **F8은 현재 "증명된 단언"이 아니라 "구조상 맞아 보이는 단언"이다.** 재현 절차: `scripts/meta-bridge-install.sh`의 `if STORE_VERDICT="$("${MIGRATE_CMD[@]}" verify 2>&1)"; then`을 `STORE_VERDICT="regressed"; if true; then`으로 바꾸고 `./run.sh check-upgrade-gate` → C6/C7 RED, F8 green(문제), 그리고 그 변조본을 sentinel PATH로 직접 실행 → 로그 생성(정상).
  ...
  - **미결**: 커밋 전이라 최종 HEAD 증거는 0건이다. commit 뒤 **3셀 증거를 최종 SHA + 그 SHA의 tgz sha256/size + container image id에 다시 묶는 것**이 남았다.
- **최종 실측 (HEAD `d102312`)**: 아래 「검수 실측」 표. **주의: 그 SHA는 강등본이다.** 되돌린 뒤의 HEAD와 3셀 아티팩트는 readiness 결함이 닫히는 SHA에서 다시 잡는다 — 지금 durable 문서로 승격한 아티팩트 수치는 없다.
