...
> **다음 세션의 정체 (GLG 2026-07-24 저녁): 구현 세션이 아니라 검수 세션이다.** 구현 줄기(C1~C4, M1, H7, pi 0.82.0, ACP 0.61.0/1.3.0, usability cut)는 오늘 전부 랜딩됐다. 지금 릴리즈하지 않으며 merge도 미룬다 — 다음 세션은 **빡세게 검수하고 문서를 검토**한 뒤 GLG에게 merge 가부를 보고한다. 아래 「검수 차터」가 그 세션의 첫 지도다. 검수 GREEN + GLG 결정 후에만 §5 마감(main NEXT #49-C 폐기 표기 → 이 파일 삭제 → merge)을 실행한다.

> **검수 세션 결과 (페블, 2026-07-24 밤 — GLG "진행 받아라" 지시로 오푸스 레인 인수): 결정론 축 전부 GREEN + F8 갭 닫힘. LIVE aggregate는 알려진 readiness race 1건만 RED — merge 가부는 GLG 결정 대기.**
> - **코드 커밋 `cbda097`** — check-upgrade-gate의 anchor-소실 legibility 수선(아래 「✅ F8 규명·수선」 참조). pre-commit full check GREEN. **3셀 재획득 (cbda097 출하 바이트)**: `pnpm check` EXIT=0(그 안에서 `check-upgrade-gate` 57/0) · `check-pack-install` EXIT=0 · `check-install-container` EXIT=0 — **tgz sha256 `db17165f962c938d71ef97cb7a86252549dc91b00d5ec67f63ec65b41039e6dc` / 11413738 bytes** / image `sha256:f1158c7f…` (게이트 수선이 `scripts/` 출하분이라 `d47fc434…`에서 이동한 것이 정상).
> - **LIVE aggregate @ `cbda097`**: `LIVE=1 ./run.sh release-gate` — **MUST 16/1/0 + BEHAVIOR 1/0, RG_EXIT=1.** 유일한 FAIL = `smoke-acp-bundled-mcp-live` — **알려진 readiness race의 표본 3** (조용한 머신, 순차 aggregate 16/17번째에서 발화 → 외부 동시부하 가설 약화). 직후 격리 재실행 PASS(14 checks, 격리 누적 4/4). 표본·모델 발화·transcript 경로는 ROADMAP 「🔴 OPEN — 번들 MCP readiness race」에 누적했다. **나머지 16 MUST 전부 PASS — `smoke-acp-v2-send-live`(발신 identity/replyability)·matrix(honest reject)·spawn-resume 포함, 브랜치 종료 정의의 delivery 증명은 최종 HEAD에서 섰다.**
> - **§5 금지 조건 5항 전부 비위반** (H7 완료 · dep 두 축 완료 · 3셀 SKIP 0 · LIVE는 최종 HEAD 실측 · 옛 pin live claim 0). readiness race는 금지 목록 밖의 **GLG 관찰 결정 항목**이지만, release-gate 자신은 MUST red로 "NOT releasable"을 말한다 — **merge와 release는 다른 게이트다. red를 안은 채 merge할지는 GLG가 정한다.** GREEN 결정 시 §5 마감 순서 그대로: main NEXT #49-C 폐기 표기 → durable 승격(BASELINE HISTORY + VERIFY floor + ROADMAP: hooksPath 발견·C6~C8 sentinel 한계) → 이 파일 삭제 → merge.

## 검수 라운드 결과 (2026-07-24 저녁, 오푸스 검수 세션 + GPT `20260724T185330-e446d2` 교차검수)

> **판정: merge 보류. 검수가 미지의 release-blocking 결함 하나를 열어냈다 — 번들 MCP readiness.** 그리고 그 과정에서 우리 자신이 그 결함을 "모델 탓"으로 두 번 오진했다가 transcript로 바로잡았다. 아래 차터 6항을 순서대로 돌린 결과이며, 원 차터는 이 절 아래에 원문 그대로 남긴다.

- **[차단·확증·미해결] 번들 entwurf-bridge MCP가 ACP 세션 tool schema에 가끔 도달하지 못한다.** 최종 HEAD `d102312`의 `LIVE=1 release-gate`가 **MUST 15/1/0, RG_EXIT=1** — `smoke-acp-bundled-mcp-live` 실패. 실패 턴의 모델 발화가 원인을 명명했다: *"도구 스키마에 `mcp__entwurf-bridge__entwurf_self`가 노출되어 있지 않습니다. 콜 가능한 도구는 Read, Bash, Edit, Write, Skill뿐입니다 … 존재하지 않는 도구 호출 결과를 지어내지 않겠습니다."* GPT가 자기 `smoke-acp-v2-send-live` 실패 표본의 세션 JSONL을 회수해 **같은 원인**을 확정했다 — `[tool:start] mcp__entwurf-bridge__entwurf_v2` → `[tool:failed] … No such tool available`. **두 표본 모두 도구가 세션 스키마에 없었다. 다만 모델의 반응은 달랐다** — v2-send 쪽은 호출을 시도했고 런타임이 실패로 답했으며, bundled 쪽은 부재를 먼저 읽고 호출하지 않았다. 행동이 갈렸다는 점이 공통 원인을 가렸다(GPT 검수 지적 — 내가 "두 번 다 호출했다"로 일반화했던 것을 여기서 정정한다).
  - **구조적 창 (GPT 소스 판독)**: `backend.ts:718-735`가 resolved config를 `newSessionArgs.mcpServers`로 정확히 넘기고 envelope enrich도 선행하지만, claude-agent-acp 0.61.0의 `createSession()`은 `await q.initializationResult()`만 기다린 뒤 즉시 반환하고(`acp-agent.js:3942-4058, 4174-4177`), 우리 backend는 model enforce 후 **곧바로 prompt**한다(`backend.ts:735-790`). **MCP connected / tools-ready를 기다리는 층이 어디에도 없다.** claude-agent-sdk 0.3.217에는 바로 그 API가 있다 — `q.mcpServerStatus()`, 상태 `pending|connected|failed|needs-auth|disabled` + connected 시 `tools[]`(`sdk.d.ts:1056-1097,2396`) — 그런데 claude-agent-acp가 createSession readiness에 쓰지 않는다.
  - **부하 상관 (인과 아님, 그리고 표본마다 조건이 다르다)**: 19:00 표본은 내 백그라운드 `pnpm check`와 **실제로 동시**였다. aggregate 표본은 앞선 라이브 턴 10여 개 뒤의 무거운 시퀀스 후반부지만, **순차 게이트는 이전 child를 종료하므로 그 실패 순간에 동시 프로세스가 있었다는 증거는 없다.** "둘 다 동시 부하"는 증거보다 강한 문장이었다(GPT 검수 지적, 여기서 낮춘다). GPT가 dist rm/build race는 배제했다(dev-location branch는 항상 TS source 실행). 격리 재실행은 bundled-mcp 3/3 PASS.
  - **처방 후보 (GLG 결정 대기)**: ① claude-agent-acp가 `createSession`에서 configured MCP별 connected + expected tool까지 bounded wait하고 실패를 ACP `newSession` error로 올리는 upstream/patch seam, ② adapter가 MCP status를 ACP 확장으로 노출하고 backend가 prompt 전 poll, ③ 재시도는 관측엔 유효하나 결정론적 readiness 계약이 아니다. **스모크만 고쳐서는 preflight를 만들 수 없다** — `mcpServerStatus()`는 claude-agent-sdk 내부 Query API이고 우리가 받는 ACP `newSession` 응답에는 없다(내 최초 처방 "턴 전에 tool list 단언"은 이 이유로 GPT가 정정했다).
- **[우리 오진 1건 — 기록으로 남긴다] 이 결함을 우리는 두 번 "모델 탓"으로 읽었다.** `7cbeb29`가 `smoke-acp-v2-send-live`를 MUST로 올린 근거는 "모델에게 어느 도구인지 TOLD했으니 자율 선택 문제가 아니다"였고, PASS/PASS/FAIL 표본을 본 우리는 그 근거가 반증됐다며 **BEHAVIOR로 강등했다**(첫 `d102312`). transcript를 읽고 나서야 강등 자체가 오진 위에 서 있었음이 드러났다 — 그 실패 표본에서 모델은 호출했고 런타임이 도구를 안 줬다(다른 표본은 부재를 읽고 호출을 거부했다 — 행동은 갈렸지만 원인은 하나다). **강등을 되돌렸다(MUST 복귀)**: advisory 레인은 모델이 *선택*하는 것을 담지, 우리 배선이 *못 주는* 것을 담지 않는다. 강등을 유지했다면 방금 잡은 결함이 advisory로 묻혔을 것이다. 죽은 `run_live_behavior_step` 헬퍼도 결합 규칙대로 함께 제거했다. **교훈: 모델 행동을 원인으로 지목하기 전에 transcript를 읽는다.** `ENTWURF_SMOKE_VERBOSE=1` 없이는 스모크가 stream을 안 남겨서 GPT는 세션 JSONL까지 가야 했다 — 실패 시 자동 stream 보존은 별도 후속 후보.
- **[중간·확증·닫힘] advisory는 공짜가 아니다 (강등 시도가 만들었던 결함, GPT가 커밋 전 포착).** `run_behavior_step`에 `env LIVE=1`을 실어 넘기면 `LIVE` 없이 `./run.sh release-gate`를 돌려도 실모델 턴이 강제된다 — opt-in 계약과 스모크 자신의 honest-skip을 동시에 깬다. 강등을 되돌리면서 그 코드는 사라졌지만, **판단은 남긴다**: 앞으로 LIVE 스모크를 advisory 레인에 넣을 일이 생기면 LIVE 검사를 자식 생성 **전에** 두고 `BEHAVIOR-SKIP`을 별도 카운터로 세야 한다.
- **[중간·확증] B 게이트의 identity 단언이 false-green 구멍이었다.** `body.includes()` 광역 매칭이라, 모델이 `entwurf_self`로 자기 gid를 알아내 payload에 적으면 봉투가 틀려도 통과 가능했다. 렌더러 separator로 header/payload를 갈라 `from:`/`session:`을 **anchored exact**로, payload를 `${nonce}` **exact**로, gid는 payload에 **부재**로 단언. 모든 보간은 regex-escape(`ENTWURF_ACP_PROVIDER_MODEL`은 운영자 입력이고 `.` 하나가 wildcard가 된다). 14→15단언.
- **[중간·확증] 릴리즈 오라클이 writer 스키마를 오표기했다.** `livewrite_schema()`가 `serializeMetaIdentity` 존재만 보고 flat `"v2"`를 찍어, 세 줄 아래 `store: v1=0 v2=0 v3=203`과 모순됐다. **1차 수선(flat `v3`)은 GPT가 반증했다** — v2 writer도 그 심볼을 갖기 때문에 flat v3는 *stale 배포 v2 번들을 current로* 오표기하며, 그건 이 섹션의 존재 이유를 반대 방향으로 깬다. `META_SCHEMA_VERSION_V3` 우선 검사 → `serializeMetaIdentity` → v1의 **3값**으로 정정하고 `check_registry_dep`의 하드코딩 "v2"도 측정값으로 교체. 실행 확인: source/assembled/installed 전부 `v3`, parity ok, EXIT=0.
- **[산문 15건, 한 뿌리] `run.sh`의 게이트 주석이 script 헤더의 사본이다.** AGENTS:271이 쓸어야 할 표면으로 "run.sh usage lines AND dispatch/function comments"를 **이름으로 명시**해뒀는데, 지난 세 라운드가 `docs/`→`scripts/`→lib만 쓸고 run.sh 사본을 남겼다. §5 금지 조건 직격(옛 pin live claim) 3건 — 그중 하나가 **핀 드리프트를 막으려고 만든 `check-dep-versions` 자신의 주석**("the 0.80 public trust exports" / "a future pi (0.81+)" — 상한은 `<0.83`)이다. 미래시제 화석 8건(`slot stays (removal is 3D-4)` 등 3D-4가 삭제한 슬롯을 usage+dispatch 양쪽이 살아있다고 말함, `check-entwurf-capabilities.ts:55`는 **자기 파일 헤더와 정면 모순**). GPT 추가 2건(`entwurf-v2-mailbox.ts:14` future 5d runner, `entwurf-control.ts:1030` — `:943`이 PI_SESSION_ID를 지우므로 "control 없이도 발신 가능"은 자기모순). **그리고 강등 자신이 만든 신규 stale 2건**(run.sh usage의 "12 LIVE smokes…/v2-send", VERIFY tier 목록)도 같은 커밋에서 닫았다.
- **재검 GREEN 항목 (되돌림 0)**: 차터 2-①(PI_SESSION_ID 소비처 — GPT 소스 실측 + 내 독립 grep 일치. 프로덕션 소비처는 pi 부모와 명시 env 주입을 받는 MCP 자식뿐이고 `acp/config.ts:304`가 ambient 값을 필터링한다. 단 문구는 **"production-controlled bash-child consumer 0"**이 정확하다 — 사용자가 bash에서 브릿지를 손으로 띄우는 비지원 경로까지 0은 아니다) · 차터 2-②(`c68aceb` 변이 재현 — GPT가 격리 worktree에서 nested `@modelcontextprotocol/sdk@1.28.0`을 심어 **원인 명명 RED**, 제거 후 GREEN 복구) · 차터 2-④(`95e6cbc` durable 수치 — `7cbeb29` detached checkout 재빌드로 tgz sha256 `ab5dee07…`·11391060 bytes·279 files·image sha 전부 **byte-exact 재현**. 이번 강등은 *현재 계약의 변경*이지 당시 측정의 오기 정정이 아니다) · 차터 3(provider/citizen 경계를 문서 절차 그대로 타이핑 재현 — plain `pi -p` EXIT=0 `ok` / 같은 모드 `entwurf_self`는 계약 문구 그대로 fail-loud / `--entwurf-control`은 자기 gid `20260724T190803-ac11c3` + replyable + socket alive) · 차터 4(H7 사후 감사 — `verify` **202 v3 non-V3=0**, backup `…-20260724T162453` **mtime 16:24 무접촉·194건(v2 191 + v3 3)** = pre-cut 바이트 보존, `doctor-meta-bridge` **PASS**, post-cut V3 mint 2건 실증).
- **[GLG 결정 2026-07-24 저녁] readiness 결함은 지금 고치지 않는다 — 옵션을 다 적어두고 더 지켜본다.** 세 처방(①upstream/patch seam ②adapter가 MCP status를 ACP 확장으로 노출 후 prompt 전 poll ③재시도)과 증거·구조 판독을 **ROADMAP 「🔴 OPEN — 번들 MCP readiness race」**에 durable로 옮겼다. 관찰 규율도 거기 있다: 재발하면 시각·동시부하·스모크·모델 발화를 표본으로 누적하고, **인과가 서기 전에는 고치지 않는다**.
- **후속 2건 랜딩 (GLG 지시로 진행)**:
  - **기존 호스트 V3 마이그레이션 사전 안내** — main이 GLG의 *모든 개발 PC*를 뜻한다는 게 확인되면서 생긴 공백이다. 다른 PC의 store는 pre-cut(v1/v2)이라 첫 세션이 identity 표면에서 fail-loud한다 — **전부는 아니다**: `entwurf_self`/`entwurf_v2`/inbox는 fail-loud하지만 `entwurf_peers`는 설계대로 listing을 유지하고 읽지 못한 레코드를 diagnostic으로 접는다(GPT 검수 지적, 여기서 정정). 반응형 처방은 이미 완비돼 있었지만(M1_PRESCRIPTION이 dev/설치본 양형식 인쇄) **사전 안내가 durable 문서에 없었다** — clean-host 워크스루는 store가 없는 호스트 기준이라 해당이 안 된다. README에 verify→migrate 절차 + backup/restore 대칭 + `--drop-parentage`가 사람의 결정이라는 점을 명시, `docs/setup-clean-host.md`에는 "이 문서는 clean 호스트 가정"이라는 경계와 함께 넣었다. **호스트당 1회.**
  - **실패 시 transcript 자동 보존** — 오늘의 오진이 가능했던 기계적 이유다. `smoke-acp-v2-send-live`는 실패 시 아무 stream도 남기지 않아(tail이 `ENTWURF_SMOKE_VERBOSE` 뒤에 있고 aggregate/CI는 그걸 안 켠다) GPT가 pi 세션 JSONL까지 파야 했다. 이제 두 ACP 스모크 모두 실패 시 **captured turn event stream을 world 밖 파일로 쓰고 경로를 인쇄**한다(best-effort — 아티팩트 쓰기가 원래 단언 오류를 가리지 않는다). bundled-mcp의 기존 30줄 tail은 유지.
- **[업그레이드 하네스 확장 랜딩 — GLG 지시 2026-07-24 밤, 구현 오푸스 / 검수 GPT `20260724T185330-e446d2`]** 위 「기존 호스트 V3 마이그레이션 사전 안내」는 **문서만** 있었고, 그건 "main이 모든 개발 PC에 퍼져도 된다"의 근거가 못 된다는 GPT 판정이 나왔다. 3셀 하네스가 그 전환을 증명하도록 확장했다.
  - **먼저 드러난 것: 검증하려던 REFUSE가 아예 없었다.** `meta-bridge-install.sh`의 preflight는 OS·python3·claude 플로어뿐이고 meta-record store를 읽지 않았다. store schema를 읽는 유일한 표면은 `meta-bridge-doctor.sh`의 사후 히스토그램. 즉 "하네스가 못 잡는 것"이 아니라 **구현이 없는 것**이었으므로 게이트를 먼저 만들었다 — 안 그랬으면 셀이 "REFUSE 없음"을 PASS로 기록했을 것이다.
  - **게이트 (`preflight_v3_store`)**: 판정을 재구현하지 않고 migrate 명령 자신의 `verify`에 위임한다(store 해석과 non-V3 집계가 거기 살고, 두 번째 구현은 두 번째 진실이 된다). absent/v3-only는 exit 0이라 clean 호스트와 이미 마이그레이션한 호스트는 특례가 필요 없다. **절대 migrate하지 않고 `--drop-parentage`를 절대 넘기지 않는다**(F7이 정적으로 고정). 진입점 3곳: `setup_all`(pnpm install·sync_auth **앞** — GPT 지적대로 `sync_auth`가 `install_local_package`보다 먼저라 install 안에만 두면 이미 `auth.json` 한 번 늦다), `install_local_package`(직접 호출 방어 — dep-integrity 뒤에 둔다. 둘 다 read-only이고, run.sh만 있는 트리는 "깨진 체크아웃"으로 실패해야 하므로), `meta-bridge-install.sh`(state prepare 앞).
  - **fixture는 셀이 만들지 않는다.** `fixtures/meta-store/`에 v2-plain·v2-parentage·v2-isEntwurf·v3·malformed **바이트 고정** + `MANIFEST.sha256`, 그리고 게이트가 같은 해시를 상수로 한 번 더 적는다(한쪽만 고치면 A2 RED — 재베이스라인은 두 파일 편집과 그걸 보는 리뷰어를 요구한다). 이유: `backup == 원본`은 "원본"이 테스트 대상이 움직일 수 없는 상수일 때만 단언이다. biome에서 `fixtures/` 제외(포매터가 정본 2-space를 탭으로 바꾸고, malformed fixture는 파싱 자체가 안 된다). `files:`에 없고 check-pack/check-pack-install의 forbidden에 `^fixtures/` 추가 — 호스트 상태이지 패키지 내용이 아니다.
  - **3셀 구성**: ① 소스 `check-upgrade-gate` **57/0** — 8개 host state(absent/empty/v3-only/v2-only/mixed/v2-parentage/malformed/mixed-problem) × 실제 `run.sh install`, refuse는 home/proj/**store/mailbox**의 **regular-file path+sha256 manifest** 무변화(퍼미션·심링크·빈 디렉터리는 범위 밖 — "전체 byte fence"라고 쓰면 과장이다), pass는 store 바이트 무변화, malformed는 자기 원인 보존 + **migrate 처방 부재**(M1이 못 고치는 상태에 migrate를 권하지 않는다), `setup`도 pnpm install 전에 refuse, backup sha256 == 고정 fixture sha256. ② `check-pack-install` — 설치본 lifecycle: 심은 pre-cut store에서 `install-meta-bridge` REFUSE(호스트 regular-file manifest 무변화 + **Claude CLI 호출 0**), 설치본 migrate가 parentage 무단 폐기 거부, `--drop-parentage` 후 backup이 고정 fixture 바이트, non-V3=0, 재설치 PASS. ③ `check-install-container` lane — V3-only 호스트 PASS(레코드 바이트 무변화) / pre-cut REFUSE→migrate→retry PASS / mixed REFUSE. fixture는 마운트가 아니라 **manifest 검증되는 base64 tar 환경변수**로 넣는다(체크아웃 비가시성 계약 유지).
  - **과대 주장 금지 (GPT 조건 수용)**: 이건 **preflight이지 lock이 아니다**. 하네스가 증명하는 것은 quiesced 호스트의 순서 계약뿐이고, 체크아웃을 직접 가리키는 개발 PC는 `세션 quiesce → pull → migrate → setup → 재개` 운영 순서가 별도로 필요하다(README에 그렇게 적었다). "V3-only untouched"도 **store 바이트**로 한정했다 — 성공한 install은 의도대로 auth/settings/XDG/Claude를 바꾸므로 호스트 전체 무접촉은 틀린 단언이고, byte fence는 REFUSE 셀에만 건다.
  - **정적 트립와이어가 내 게이트를 먼저 잡았다.** `check-install-surface` S5c가 헬퍼 함수 뒤에 숨은 샌드박스를 못 본다고 RED — 그게 S5c의 존재 이유다(“한 가지 문법만 보는 트립와이어는 다음 유출이 지나가는 길”). 게이트를 top-level `export` 블록 + 케이스별 reset 구조로 바꿔 정적으로 읽히게 했다.
  - **GPT 검수 라운드 (필수 4건, 전부 닫음 — mutation으로 증명)**:
    - **[false-green] 소스 셀의 무접촉 fence가 store를 안 봤다.** `host_bytes()`가 `find home proj`뿐이라 store/mailbox는 sandbox home의 *형제*여서 fence 밖이었다 — refusal이 store를 훼손해도 GREEN. **mutation 증명**: refusal 직전에 store에 파일을 하나 쓰게 만들면 수정 후 fence는 `❌ B/v2-only refusal mutated the host before refusing`, 같은 mutation을 옛 `find home proj` fence로 돌리면 `✅ … left every persistent regular file unchanged` — 정확히 GPT가 말한 통과였다. fence에 store/mailbox 포함, 문구도 "전체 byte fence"에서 **regular-file path+sha256 manifest**로 낮췄다(퍼미션·심링크·빈 디렉터리 범위 밖).
    - **[오처방] malformed store에 migrate를 권하고 있었다.** 원인은 보존했지만 wrapper가 무조건 "Migrate the store …"로 끝났다. malformed는 M1 대상이 아니다 — 블랙아웃 중인 운영자에게 못 고치는 명령을 쥐여주는 셈. 진단에 따라 분기(`pre-cut v[0-9]* record` 있으면 migrate 처방, 아니면 "NOT a migration case — 이 레코드들은 migration이 변환하지 못한다")하고 C3c(처방 부재)·C3d(그 사실을 말로 함) 신설. **mutation 증명**: 무조건 처방으로 되돌리면 C3c/C3d 둘 다 RED.
    - **[hard rule 10] 새 셸 게이트가 설치본에서 legible REFUSE를 안 했다.** `fixtures/`는 출하 제외지만 `scripts/`는 통째로 출하되고, `check-upgrade-gate`는 셸이라 `run_ts`의 거부를 안 탄다. **mutation 증명**: node_modules 레이아웃을 흉내 내 돌리면 가드 없이는 `cd: …/fixtures/meta-store: No such file or directory`, 가드 추가 후엔 `dev-clone-only surface` + exit 1. 스크립트 자체 가드 + `check-pack-install`에서 설치본 구동 단언 추가.
    - **[문서] 과장 3건.** README "You will not reach that state by accident"는 바로 아래 pull 경고와 모순 → "installer entrypoints will not cross that boundary silently"로 한정. NEXT의 "전 identity 표면 fail-loud"(peers는 살아서 diagnostic으로 접는다) 및 "전체 byte fence" 정정.
    - **추가 강화 (GPT 판정 1 수용)**: `meta-bridge-install.sh`의 toolchain 게이트를 둘로 쪼개 **store 게이트를 외부 `claude` CLI 접촉 앞으로** 옮겼다. 이제 fence가 "mutating 서브커맨드 0"이 아니라 **claude 호출 0건**이다(셀2·셀3 둘 다 로그 empty 단언). F6도 주석 줄 대신 **실제 verify 실행 줄**을 앵커로 바꿨다.
    - **[GPT 후속 검수 — mixed-problem 처방 순서]** pre-cut과 malformed는 배타적 상태가 아니다. 둘이 함께 있으면 옛 2분기는 `pre-cut` substring만 보고 migrate를 즉시 권하지만, 실제 migrate는 problem 때문에 반드시 REFUSE했다. 처방을 `problem=0+pre-cut → migrate` / `problem+pre-cut → problem 복구 FIRST, 그 뒤 migrate` / `problem-only → NOT migration`으로 나누고 `mixed-problem` host state를 추가했다. run.sh와 별도 출하되는 meta installer 양쪽을 C4~C6가 구동한다. host composition authority인 `hosts.json`도 MANIFEST+gate constant 이중 hash에 편입했다.
  - **GPT 후속 검수 (오푸스 수선의 결함 1건 — 실재)**: **pre-cut과 malformed는 배타적이지 않다.** 오푸스가 2분기(pre-cut substring이면 migrate 처방, 아니면 NOT-a-migration)로 고쳤는데, 둘이 함께 있는 store에서 verify는 migrate 처방을 **인쇄하지만** `cmdMigrate`는 `problems.length>0`에서 반드시 REFUSE한다 — 즉 운영자에게 반드시 실패하는 명령을 쥐여준다. 실측 확인: mixed-problem store에서 `verify`는 `pre-cut v2 record ×1 … migrate with …` + `not valid JSON` + `0 v3 / 1 v2 / 0 v1 / 1 problem(s)`, `migrate`는 `REFUSE: … nothing was written, no backup taken` rc=1. GPT가 **3축 처방**(problem=0+pre-cut→migrate / problem>0+pre-cut→**repair FIRST** / problem-only→NOT a migration case)으로 고치고 `pre-cut v[0-9][0-9]*`로 정규식을 조였으며, `mixed-problem` host state와 C4~C8을 신설했다. **mutation 증명**: 3축을 2분기로 되돌리면 C4/C5가 RED(`54/2`). 또한 `hosts.json`이 composition authority인데 MANIFEST가 record 바이트만 해시하던 구멍도 GPT가 닫았다(manifest 편입 + 게이트 상수 이중 spelling + seeder 선행 전체 검증).
  - **회귀 시 real Claude 실행 관측 → GPT가 봉인**: `check-upgrade-gate` 상단에 **PATH-local `claude` sentinel**(나머지 PATH 유지 → node/python3/sha256sum 의미 불변). C6~C8이 store 게이트 회귀로 Claude까지 낙하하면 sentinel이 argv를 기록하고 exit 97, **F8**이 그 로그의 부재를 단언한다. 내가 "깨끗한 봉인이 없다"고 적었던 항목을 GPT가 닫았다.
  - **최종 재획득 (sentinel 포함 바이트 기준, 오푸스 실행 — 전부 GREEN)**: `pnpm check` **EXIT=0** · 소스 셀 `check-upgrade-gate` **57 passed / 0 failed** · `check-pack-install` **EXIT=0** · `check-install-container` **EXIT=0, 43 ok / 0 FAIL** · `git diff --check` clean. 커밋 직전 워킹트리 아티팩트: tgz **`d47fc434779cd5d1fbbd29c00820fcbd9679a61cb4d53c4c556d3c01e35cd1c5`** / **11413529 bytes** / image `sha256:f1158c7f34cf35a047bf0513c38282bb2fa253529e5ae404b32c6d93697410be`. (이전 라운드의 tgz 잠정값들은 전부 폐기 — 바이트가 계속 움직였다. `docs/setup-clean-host.md`도 `files:`에 있어 문서 수정만으로도 tgz가 바뀐다.)
  - **✅ 최종 SHA 바인딩 완료 — `9c29234095969b1c3b4520e1351bcfa2f873560d` (`feat(upgrade): refuse a pre-cut host before any write, and prove it in all three cells`).** 이 SHA에서 재획득한 값이 durable 증거다: **tgz sha256 `d47fc434779cd5d1fbbd29c00820fcbd9679a61cb4d53c4c556d3c01e35cd1c5` / 11413529 bytes**, container image `sha256:f1158c7f34cf35a047bf0513c38282bb2fa253529e5ae404b32c6d93697410be`, `check-pack-install` **EXIT=0**, `check-install-container` **EXIT=0 / 43 ok / 0 FAIL**. `pnpm check`는 pre-commit 훅이 통째로 돌아 통과했고(그 안에서 `check-upgrade-gate` **57/0**), 커밋 후 워킹트리 clean. **주의: 이 파일(`NEXT--repair_v2-core-debt.md`)은 `files:`에 없어 출하되지 않으므로, 이 문서를 더 고쳐 커밋해도 위 tgz sha256은 움직이지 않는다.** 반대로 `run.sh`·`docs/setup-clean-host.md`·`scripts/`는 출하되므로 그쪽을 한 글자라도 고치면 재획득이 필요하다. **아직 push하지 않았다** (GLG authority).
  - **✅ F8 규명·수선 완료 (2026-07-24 밤, 페블 검수 세션).** 모순의 실체는 F8의 결함이 아니라 **게이트가 F8까지 도달하지 못한 것**이었다. 위 재현 절차의 변조는 F6 앵커 문자열(`"${MIGRATE_CMD[@]}" verify`)까지 없애는데, `set -euo pipefail` 아래에서 `mb_gate=$(grep …|head|cut)`의 grep 무매치(rc=1)가 **할당문 자체를 죽여** 게이트가 F5 직후 아무 진단 없이 종료된다 — F6~F8과 요약 줄이 인쇄되지 않는다(실측: 출력이 F5에서 끝, GATE_EXIT=1. false-green은 아니고 **illegible死**). 즉 anchor 소실을 잡으라고 있는 `bad "F4/F6 … missing"` 분기가 도달 불가능한 죽은 코드였다. **수선**: 실패 가능한 할당-치환 6곳(F4 `gate_ln`, F6 `mb_gate`/`mb_write`, D4 `got`, E6 `parent_sha`/`entwurf_sha`)에 `|| true` — 실패가 빈 값으로 흘러 기존 비교 분기가 말로 실패한다(awk/find 기반과 `set +e` 래핑 구간은 원래 안전 — 전수 확인). **mutation 재증명 2종**: ① 같은 변조에서 이제 C6/C7 RED + F6 RED(`verify= prepare=165`) + **F8 RED(sentinel `--version` ×2 = C6+C7 발화 검출)** + 요약 `53/4` 인쇄, EXIT=1. ② 앵커를 보존한 우회 변조(`: "${MIGRATE_CMD[@]}" verify`)에서도 게이트가 F8까지 완주하고 **F8 RED** — 게이트가 F8에 도달하기만 하면 sentinel을 정확히 읽는다는 것이 두 경로에서 실측됐다. 격리 실행의 sentinel 발화(`--version`, rc=1)도 재확인. **F8은 이제 증명된 단언이다** — 회귀는 F8(도달 시) 또는 F6(앵커 소실 시)이 반드시 legible하게 잡는다.
  - **남은 관측 1건 (블로커 아님)**: C6~C8은 실제 `meta-bridge-install.sh`를 구동한다. 지금은 store 게이트가 claude 접촉 앞이라 항상 그 전에 죽지만, **store 게이트가 회귀해 통과하면** 그 드라이브가 운영자의 진짜 `claude` 바이너리를 실행한다(쓰기는 샌드박스 HOME 안, 게이트는 어차피 RED). 깨끗한 봉인이 없다 — PATH에서 claude만 빼면 node/python3까지 잃고, fake claude를 주면 C6~C8의 의미가 바뀐다. 고치지 말고 이 사실을 아는 채로 둔다.
  - ~~**미결**: 커밋 전이라 최종 HEAD 증거는 0건이다.~~ → 위 ✅ 바인딩(`9c29234`)으로 닫혔고, 게이트 수선 후 **`cbda097`에서 재바인딩 완료**(NOW의 「검수 세션 결과」 — tgz `db17165f…`).
- **최종 실측 (HEAD `d102312`)**: 아래 「검수 실측」 표. **주의: 그 SHA는 강등본이다.** ~~되돌린 뒤의 HEAD와 3셀 아티팩트는 readiness 결함이 닫히는 SHA에서 다시 잡는다~~ → GLG의 "고치지 않고 관찰" 결정으로 전제가 바뀌어, race를 안은 채 **`cbda097`에서 3셀+LIVE를 재획득했다**(NOW 참조). durable 승격은 §5 마감에서.

### 원 차터 (2026-07-24, 실행 완료 — 무엇을 보라고 했는지의 기록)

1. **최종 HEAD에서 3셀 + LIVE 재획득.** (c) 증거는 `7cbeb29` 바이트에 묶여 있고 이후 docs 커밋들로 HEAD가 움직였다(당시 판단: docs-only라 재획득 불필요 — 오푸스, 페블 동의. **이 판단 자체를 검증하라**). 주의: `run.sh`는 tarball에 들어가므로 docs 커밋이어도 tgz sha256은 움직인다 — 재획득 시 새 sha를 그 HEAD SHA에 묶어 기록. `LIVE=1 ./run.sh release-gate <mkdir한 fresh-scratch>` 기대값 = MUST 17 + BEHAVIOR 1. **실측은 MUST 15/1/0 + BEHAVIOR 1/0, RG_EXIT=1**이었다 — 그 "docs-only라 재획득 불필요" 판단은 **틀렸다고 판정**한다. 재획득하지 않았다면 번들 MCP readiness 결함을 모른 채 main에 들어갔을 것이다. 차터 1항이 이 세션의 값을 혼자 정당화했다.
2. **오늘 커밋 9개 개별 검수** (`0f7db49`(backup 완결성)·`0620ce1`·`a4a19be`(domain 단일화)·`c1f3d21`(M1 동결)·`9437c14`·`01e7235`·`aad2e27`(H7 원장) — 페블 / `dfa3967`(pi 0.82.0)·`5f5a18d`(ACP)·`c68aceb`(D)·`7cbeb29`(A′+B)·`95e6cbc`(durable) — 오푸스). 특히: ① 두 dep cut의 seam 판정 재검(0.82.0 bash tool의 PI_SESSION_ID 탈취 → "entwurf 소비 0건" 주장 독립 grep), ② `c68aceb` 변이 재현(nested 1.28.0 심고 RED 확인), ③ **`smoke-acp-v2-send-live`의 자기충족 여부** — sender gid가 프롬프트에 없다는 non-circular 주장을 소스에서 확인하고 flake 표본 3회+ 추가(MUST 승격이 성급했으면 BEHAVIOR로 강등 제안), ④ `95e6cbc` durable 문서의 수치가 실측과 일치하는지.
3. **문서 빡검 (§5 금지 조건 기준).** 옛 pin(0.80.x/0.81.x/0.54.x/1.1.0/0.3.197) live claim repo-wide 두 축 grep **독립 재실행**(오푸스가 0건 보고 — 재검), README/AGENTS/VERIFY/BASELINE/setup-clean-host 교차 정합, **provider vs citizen 경계 문서를 절차 그대로 타이핑해 재현**(plain `pi -p` = provider smoke 성공 + entwurf 도구 fail-loud / `--entwurf-control` = citizen 성공).
4. **H7 사후 감사.** 라이브 store `verify` 194+ v3 non-V3=0, backup `~/.pi/agent/meta-sessions.v3-migration-backup-20260724T162453` 실존·무접촉, doctor PASS 재실행, 새 세션 V3 mint 확인. M1 동결 4문장이 코드와 여전히 일치하는지(§6).
5. **GPT 검수 라인 병행.** `20260724T161741-d294f0`(alive면) 최종 검수 예약이 서 있다 — 교차검수 결과가 갈리면 GLG로. 채널은 이제 entwurf_v2로 직접 쓴다(수동 릴레이 불필요).
6. **판정 보고.** 검수 GREEN이면 merge-ready를 GLG에 보고(merge/release는 GLG authority). 결함 발견 시 수선 후 재검 — 오늘의 방식대로 실재현이 판정을 이끌게 하라.
...
### 4. Final delivery acceptance — 최종 pin·최종 바이트에서 전부 다시 → **DONE @ `7cbeb29` (2026-07-24: 3셀 SKIP 0 + LIVE MUST 17/17 + BEHAVIOR 1/1, tgz `ab5dee07…` — 이후 docs 커밋으로 HEAD 이동, 재획득 판단은 검수 차터 1)**
...
### 5. Durable docs + merge cleanup — branch에서 끝낸 뒤 잊는다 → **durable 승격은 DONE(`95e6cbc`), merge cleanup(#49-C 폐기 표기·이 파일 삭제·merge)은 검수 GREEN + GLG 결정 뒤**
