...
| `ENTWURF_REQUIRE_DOCKER=1 check-install-container` | **EXIT=0** — Node 24 checkout-invisible consumer. **커밋 `9c8ee58` 바이트 tgz sha256 = `0d53ad5b…`** (container·수동 pack·dist 재빌드·mtime 변경 전부에서 결정론적 재현 실측). GPT 핸드오프가 적어둔 `c21233ef…`는 커밋 전 워킹트리 값이라 최종 커밋에서 재현되지 않는다 — sha256을 박을 땐 반드시 커밋 SHA에 묶는다 |
...
6. **M1 + H7 레인 (라이브 cutover)**: ~~M1 operator command~~ → **M1 DONE (2026-07-24, 위 NOW 참조)** — backup `<store>.v3-migration-backup-<ts>/` → migrate → verify non-V3=0 → restore 전부 코드+70단언 게이트로 성립. fixture: V1/V3-already/malformed/stray-key/mismatch(drift)/half-migrated **+ duplicate nativeSessionId** + restore real-sibling-only(foreign/nested/forged-suffix/symlink 거부). **남은 것 = H7 라이브 전환.**
   ...
   **⚠️ 실행 체크리스트는 아직 미완 (GPT M1-후속 #3, H7 방아쇠 전 필수).** 위는 *불변식*이지 copy/paste 가능한 명령 순서가 아니다. H7 전에 아래를 명령 수준으로 조여야 하고, ★ 표시는 GLG 환경 지식이 필요한 빈칸이다:
   ...
   - **⑥ 실패 시 롤백** — `entwurf meta-bridge-migrate-v3 restore <그 store의 .v3-migration-backup-<ts>>` (이 store의 sibling만 받는다 — #1 수선; store만 되돌리고 mailbox state는 state-wins·멱등) → 구 런타임 재설치. 어느 단계에서 restore로 꺾을지 기준을 ★ 미리 정한다.
