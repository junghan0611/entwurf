...
- **[차단·확증·미해결] 번들 entwurf-bridge MCP가 ACP 세션 tool schema에 가끔 도달하지 못한다.** 최종 HEAD `d102312`의 `LIVE=1 release-gate`가 **MUST 15/1/0, RG_EXIT=1** — `smoke-acp-bundled-mcp-live` 실패. 실패 턴의 모델 발화가 원인을 명명했다: *"도구 스키마에 `mcp__entwurf-bridge__entwurf_self`가 노출되어 있지 않습니다. 콜 가능한 도구는 Read, Bash, Edit, Write, Skill뿐입니다 … 존재하지 않는 도구 호출 결과를 지어내지 않겠습니다."* GPT가 자기 `smoke-acp-v2-send-live` 실패 표본의 세션 JSONL을 회수해 **같은 원인**을 확정했다 — `[tool:start] mcp__entwurf-bridge__entwurf_v2` → `[tool:failed] … No such tool available`. **모델은 두 번 다 호출했다.**
  ...
  - **부하 상관 (인과 아님)**: 두 실측 모두 무거운 동시 부하 아래였다(GPT 19:00 표본은 내 백그라운드 `pnpm check`와 겹쳤고, aggregate는 앞선 라이브 턴 10여 개가 같은 조건을 만든다). GPT가 dist rm/build race는 배제했다(dev-location branch는 항상 TS source 실행). 격리 재실행은 bundled-mcp 3/3 PASS.
...
- **[우리 오진 1건 — 기록으로 남긴다] 이 결함을 우리는 두 번 "모델 탓"으로 읽었다.** `7cbeb29`가 `smoke-acp-v2-send-live`를 MUST로 올린 근거는 "모델에게 어느 도구인지 TOLD했으니 자율 선택 문제가 아니다"였고, PASS/PASS/FAIL 표본을 본 우리는 그 근거가 반증됐다며 **BEHAVIOR로 강등했다**(첫 `d102312`). transcript를 읽고 나서야 강등 자체가 오진 위에 서 있었음이 드러났다 — 모델은 호출했고 런타임이 도구를 안 줬다. **강등을 되돌렸다(MUST 복귀)**: advisory 레인은 모델이 *선택*하는 것을 담지, 우리 배선이 *못 주는* 것을 담지 않는다. 강등을 유지했다면 방금 잡은 결함이 advisory로 묻혔을 것이다. 죽은 `run_live_behavior_step` 헬퍼도 결합 규칙대로 함께 제거했다. **교훈: 모델 행동을 원인으로 지목하기 전에 transcript를 읽는다.** `ENTWURF_SMOKE_VERBOSE=1` 없이는 스모크가 stream을 안 남겨서 GPT는 세션 JSONL까지 가야 했다 — 실패 시 자동 stream 보존은 별도 후속 후보.
