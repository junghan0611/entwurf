...
> GPT 검수 4건과 그 뒤 핸드오프 검수가 찾은 restore guard tail까지 `9c8ee58`에서 닫혔다. sibling symlink→foreign과 forged suffix를 직접 RED로 만들고, real-directory timestamp sibling만 허용했다(S15). 그 위에서 **GPT 재검수(2026-07-24)가 이름 검증의 다음 층을 재현**했다 — "이름이 올바른가"는 닫혔지만 "그 이름의 backup이 완전하고 판독 가능한가"가 열려 있었다: 유효한 이름의 malformed backup이 rc=0으로 복원돼 malformed record가 주소 권위가 됐다(직접 synthetic 재현). **`0f7db49`(페블)가 닫았다**: ① migrate backup을 `.partial-<pid>` staging에 복사 후 atomic rename으로만 final name을 부여(부분 복사가 trusted name을 차지 불가, 실패는 원인과 함께 loud REFUSE), ② restore가 store를 움직이기 전에 backup을 classifyStore()로 검사(problem 0 + record ≥1; v1/v2/v3 전부 합법 — backup은 pre-cut 바이트), ③ record 항목은 regular file만(classify는 링크를 따라 읽는데 copy는 링크를 심는 어긋남 차단), ④ lstat 실패가 실제 원인(EACCES/ENOTDIR 등)을 보존. 게이트 S16(내용 검증·lstat 원인·symlink record)+S17(PATH_MAX로 강제한 mid-copy 실패·staging 잔재 거부) 신설, **70→80단언**. full check + pre-commit hook aggregate GREEN. **다음 세션이 새로 파야 할 첫 축은 H7 준비**이며, 닫힌 수선을 다시 반복하지 않는다.

- **M1 검수 3라운드 종결 (2026-07-24).** 랜딩 `04f55b5`(페블) → 오푸스 R1 검수(결함 M1~M6, 되돌림 0, 판단 3건 전부 동의) → 페블 수선 `2a4fe13`/`95c15ff` → 오푸스 R2 검수(M1·M2·M3 닫힘 확인, 잔여 3건 R1~R3) → **오푸스가 직접 마감(`9e0ae46` 등)** → **GPT 교차 검수 4건(아래) 처리(`33ce266`) + 핸드오프 검수 restore tail 수선(`9c8ee58`) + GPT 재검수 backup 완결성 수선(`0f7db49`)**. 게이트 `check-meta-migrate-v3` **80단언**, 독립 full check EXIT=0.
...
- Current: **세 문장 목표 ①②③ + M1은 현재 pin에서 코드로 성립하고 교차검수·restore tail·backup 완결성 수선을 통과했다.** 현재 알려진 push 차단 결함은 0이다. 남은 줄기는 **push(checkpoint) → H7(current-pin baseline) → pi uplift → ACP uplift → final artifact delivery acceptance → main merge**다.
...
### 현재 실측 (2026-07-24, backup 완결성 수선 커밋 `0f7db49` 바이트)

| 검증 | 결과 |
|---|---|
| 독립 full `pnpm check` | **EXIT=0** — 앞선 2398 + S16/S17 10개 = **2408 회계** (pre-commit hook aggregate도 GREEN) |
| `check-meta-migrate-v3` | **80단언** — S15(이름·grammar·symlink dir) 위에 S16(malformed/empty/drift 내용·symlink record·lstat 원인)+S17(mid-copy 실패·staging 잔재) mutation |
| `./run.sh check-pack-install` | **EXIT=0** — 설치본 bin/compiled twin 포함(migrate CLI twin이 이번 수선의 실변경물) |
| `ENTWURF_REQUIRE_DOCKER=1 check-install-container` | **EXIT=0** — Node 24 checkout-invisible consumer. **커밋 `0f7db49` 바이트 tgz sha256 = `620eed20…`** (직전 `9c8ee58`의 `0d53ad5b…`에서 이동한 것이 정상 — CLI/게이트가 바뀌었다). sha256은 반드시 커밋 SHA에 묶는다 |
...
6. **M1 + H7 레인 (라이브 cutover)**: ~~M1 operator command~~ → **M1 DONE (2026-07-24, 위 NOW 참조)** — backup `<store>.v3-migration-backup-<ts>/` → migrate → verify non-V3=0 → restore 전부 코드+80단언 게이트로 성립. fixture: V1/V3-already/malformed/stray-key/mismatch(drift)/half-migrated **+ duplicate nativeSessionId** + restore real-sibling-only(foreign/nested/forged-suffix/symlink 거부) **+ backup 완결성(staging 원자성·내용 classify·regular-file record·lstat 원인 보존)**. **남은 것 = H7 라이브 전환.**
   ...
   **⚠️ 실행 체크리스트는 아직 미완 (GPT M1-후속 #3, H7 방아쇠 전 필수).** 위는 *불변식*이지 copy/paste 가능한 명령 순서가 아니다. H7 전에 아래를 명령 수준으로 조여야 하고, ★ 표시는 GLG 환경 지식이 필요한 빈칸이다:
   - **① quiesce** — 설치본 v2 writer를 전부 멈춘다: Claude Code `SessionStart` 훅(`~/.claude/settings.json` → `~/.local/share/entwurf/meta-bridge/.assembled`)과 agy `PreInvocation`이 세션마다 v2를 민팅 중. ★ 어느 프로세스/훅을 어떤 순서로 끄고, 정지를 무엇으로 확인하나(민팅 멈춤 = `verify` 카운트가 두 번 연속 불변).
   - **② verify (read-only)** — `entwurf meta-bridge-migrate-v3 verify`: non-V3 카운트 + parentage 처분 필요 여부를 blackout 전에 확인(현재 실측 v2 189 / v3 1 / problem 0 / parentage 0 → `--drop-parentage` 불필요).
   - **③ exact candidate 고정** — ★ 어떤 tgz/HEAD를 설치하나(릴리즈 컷 여부 = GLG). cut 직전 ids/HEAD/patch/sha256 고정.
   - **④ 원자 전환** — `migrate`(backup 자동) → non-V3=0 확인 → **즉시** 새 V3-only 런타임 설치. ①의 writer는 이 순간까지 죽어 있어야 한다(불변식).
   - **⑤ 판정** — 설치 후 복구 증거: `doctor-meta-bridge` 0 + `entwurf meta-bridge-migrate-v3 verify` non-V3=0 + **라이브 형제 채널 왕복 1회**(pi↔Claude 양방향 delivery — 지금 죽어 있는 그 채널이 되살아나는 것이 H7의 성공 정의). ★ 어느 두 시민으로 왕복을 칠지.
   - **⑥ 실패 시 롤백** — `entwurf meta-bridge-migrate-v3 restore <그 store의 .v3-migration-backup-<ts>>` (이 store의 **완전 판독 가능한** sibling만 받는다 — 이름 grammar + 내용 classify, `9c8ee58`/`0f7db49` 수선; store만 되돌리고 mailbox state는 state-wins·멱등) → 구 런타임 재설치. 어느 단계에서 restore로 꺾을지 기준을 ★ 미리 정한다.
