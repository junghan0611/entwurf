/**
 * entwurf-v2-lock — the per-gid dispatch lock primitive (0.11 Stage 0 step 5a, 버킷 B F2), proven beside the module it guards (#119 V3).
 *
 * MIGRATED from scripts/check-entwurf-v2-lock.ts. Transcribed, not rewritten: every label is kept verbatim
 ...
 * The temp dir is REAL, not faked: the openSync(lockPath,"wx") OS-level atomicity is the thing under test. Only the content-shaping deps (clock / nonce / pid / hostname / kill) are injected.
 *
 * The original header, unchanged — check-entwurf-v2-lock — deterministic gate for 0.11 Stage 0 step 5a (버킷 B F2):
...
import { ENTWURF_V2_REJECT_REASONS } from "./entwurf-v2-contract.ts";
import {
	acquireLock,
...
it("no drift: the conflict reason IS the contract's target-locked", () => {
...
it("F2-P1: malformed gid throws before building a path", () => {
	for (const bad of ["", "../escape", "not-a-gid", "20260612T101010-AAAAAA", "20260612T101010-aaaaa"]) {
		expect(() => lockPathFor(bad), `F2-P1: lockPathFor('${bad}') refuses to build a path`).toThrow(/invalid garden id/);
	}
...
it("classifyProcessLiveness: ESRCH=dead, EPERM=denied, ok=alive, unknown=alive", () => {
...
it("acquire writes a well-formed claim", () => {
	withTempDir((dir) => {
...
it("mutual exclusion: a second acquire without release = target-locked", () => {
	withTempDir((dir) => {
...
it("release: a successor's claim (different nonce) survives a late release", () => {
	withTempDir((dir) => {
...
it("stale reclaim policy: same host + ESRCH only", () => {
	withTempDir((dir) => {
...
it("empty/corrupt lockfile → conflict, never auto-deleted", () => {
	withTempDir((dir) => {
		...
		if (!res.ok) {
			...
			ok(
				"corrupt: detail carries file mtime (only age signal when body unreadable)",
				/mtime/.test(res.conflict.detail),
			);
		}
...
it("L3: gardenId mismatch (path authority = gid) → conflict, not reclaimed", () => {
	withTempDir((dir) => {
		const lockPath = lockPathFor(GID_A, dir);
		fs.mkdirSync(dir, { recursive: true });
...
it("L1: reclaim under a wx mutex — closes the F2 two-reclaimer race", () => {
	withTempDir((dir) => {
		const lockPath = lockPathFor(GID_A, dir);
		const held = acquireLock(GID_A, fixedDeps({ dir }));
	...
	withTempDir((dir) => {
		...
		if (!blocked.ok)
			ok(
				"reclaim-mutex: marker conflict detail carries marker mtime (O2)",
				/marker mtime/.test(blocked.conflict.detail),
			);
	});
});

// ── L3 release: gardenId mismatch (path authority) → not-owned, preserved ──
it("L3 release: gardenId mismatch (path authority) → not-owned, preserved", () => {
	withTempDir((dir) => {
		const lockPath = lockPathFor(GID_A, dir);
		fs.mkdirSync(dir, { recursive: true });
