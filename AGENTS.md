...
- **Every repair/removal commit hunts the sentences it invalidated** — adding the new fact without deleting the old claim has now shipped three times (dead registry link → stale NEXT diagnosis → A1/launcher prose surviving C4). The sweep target list, all of it, every time: README · AGENTS · DELIVERY · VERIFY · **ROADMAP · run.sh usage lines AND dispatch/function comments · the doc comments of every module the commit touched**.
