...
13. **A native-hook owner is explicit, not a topology guess.** Claude command hooks do not expose one portable process tree: under the same Claude Code version we observed both a direct hook→Claude join and a retained `/bin/bash -c` wrapper. Ordinary shell tail-exec tests do not reproduce the difference, so its trigger inside Claude's hook spawn path remains unknown; no OS or host is declared safe. The meta-bridge command captures shell `$PPID` as `ENTWURF_META_HOOK_OWNER_PID` before `exec`, and the hook trusts it only when it is actually in the hook's ancestry. Never revert to `process.ppid` alone or blindly write both parent and grandparent: the former can key a transient wrapper, while the latter can key the long-lived login shell and resurrect a dead receiver. The deterministic gate must drive both retained-shell and tail-exec topologies. **The doctor is the release oracle, so its exit 0 must mean every required layer was measured, never that a layer was skipped.** It resolves the ONE artifact Claude loads (`claude plugin list --json`.installPath; an ambiguous multi-version cache is refused, never guessed), classifies the installed *launch form* by name across all three owner hooks — a recognized exec-form is reported as `not authorized by the current release contract`, not as unreadable drift — and then requires the live MCP↔marker join. Missing live evidence is `NOT CERTIFIED`, a failure worded distinctly from a broken install; macOS is install-supported but cannot reach that tier (no `/proc` for bridge discovery) and is a release blocker, not a pass. `check-meta-doctor-oracle` holds this: a healthy fixture must reach PASS and nineteen planted defects must each turn it red *naming their own cause*. An oracle with an optional central evidence layer is not an oracle.
...
./run.sh check-meta-doctor-oracle           # detection power of the release oracle: healthy fixture reaches `doctor: PASS`, 19 planted defects each turn it FAIL naming their own cause
