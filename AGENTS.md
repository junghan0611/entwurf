...
- **Every repair/removal commit hunts the sentences it invalidated** — adding the new fact without deleting the old claim has now shipped **seven** times (dead registry link → stale NEXT diagnosis → A1/launcher prose surviving C4 → `docs/setup-clean-host.md` teaching the deleted `--session-id` launcher *and a hard-exit guard that no longer exists* → the `shouldListAsLive` removal leaving its own policy sentences behind → the store-doctor claiming dual-read three cuts after V3-only · a gate header claiming v2 writes — both surviving FOUR sweeps because no commit touched those files). Three times the miss was a surface the previous version of THIS list did not name, so the list is the fix, not the reminder. All of it, every time: README · AGENTS · DELIVERY · VERIFY · **ROADMAP · `docs/**` (setup/rail guides — an install walkthrough is a new host's first surface, so a stale command there costs more than a stale README line) · run.sh usage lines AND dispatch/function comments · the doc comments of every module the commit touched, including the gate scripts · `scripts/**` operator & gate scripts BY NAME (grep the cut's vocabulary — dual-read, v2, the deleted symbol — across ALL of scripts/, not only the files the commit touched: a schema/authority cut invalidates prose in consumers it never edits)**. A deleted symbol's *prose* is not deleted by the compiler or by biome — only by this sweep.
