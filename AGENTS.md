...
11. **Verification must not rewire the operator's own install.** An offline smoke that writes a live `~/.claude` / `~/.gemini` / `~/.pi` path uninstalls the operator as a side effect of "testing". Swap both `HOME` **and an already-exported `XDG_DATA_HOME`** to the sandbox: moving HOME alone still writes real install-state below the inherited XDG root. `check-install-surface` S5 is a static **tripwire** over shell source, not a proof: it catches a literal live path and one hop of aliasing, and it cannot see a path assembled across variables or built inside an embedded heredoc. Read a green S5 as "no obvious destructive line", never as "verification is sandboxed" — the real guarantee is running the offline floor under a swapped HOME+XDG, which is still open. LIVE gates are the only surfaces that may drive the real host, and they say so in their name.
12. **A doctor reports runtime truth and ownership truth separately.** Read the target's own semantics before calling a host broken. agy matches `mcp(*)` and `mcp(<server>)` against our tool wherever those rules appear, so an operator's broad `allow` already grants `entwurf_v2` — reporting that host as "NOT granted, agy prompts on every call" was a false red about a working surface. Installers still take the narrowest rule they need; doctors distinguish **we own this** from **someone else's rule is carrying it** from **it is genuinely broken**. Install-state is evidence only when it parses, names its required managed-path field, and that normalized path equals the live target this host reads; corrupt or foreign-target state is a failure even when the live command itself resolves. Conversely, broken ownership state does not justify saying a visibly configured runtime command is absent — report both axes honestly and keep the final verdict red.
...
./run.sh smoke-agy-install-state            # MCP + exact permission ownership + honest inverse (136)
./run.sh smoke-agy-statusline-state         # ambient garden identity install surface (68)
./run.sh smoke-agy-hooks-state              # PreInvocation birth/sender hook install surface (43)
