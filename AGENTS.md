...
11. **Verification must not rewire the operator's own install.** An offline smoke that writes a live `~/.claude` / `~/.gemini` / `~/.pi` path uninstalls the operator as a side effect of "testing". Swap both `HOME` **and an already-exported `XDG_DATA_HOME`** to the sandbox: moving HOME alone still writes real install-state below the inherited XDG root. `check-install-surface` S5 is a static **tripwire** over shell source, not a proof: it catches a literal live path, one hop of aliasing, and (S5b) a smoke that swaps HOME without swapping XDG_DATA_HOME — but it cannot see a path assembled across variables or built inside an embedded heredoc. Read a green S5 as "no obvious destructive line", never as "verification is sandboxed" — the real guarantee is running the offline floor under a swapped HOME+XDG, which is still open. LIVE gates are the only surfaces that may drive the real host, and they say so in their name.
