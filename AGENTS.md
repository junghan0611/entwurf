...
9. **Identity carrier + whitelist overlay design**: borrow each backend's model/API/tools; shape only the pi-facing surface.
   ...
   - **Compaction / evidence (0.5.0)**: The bridge does not implement compaction. ACP backends compact natively; the pi session survives that. Defaults — pi JSONL compaction blocked, backend-native compaction allowed (no guard injected). Knobs — `PI_SHELL_ACP_ALLOW_PI_COMPACTION=1` opts back into pi-side compact (rare; pi-side summary does not reduce the backend transcript); `PI_SHELL_ACP_DISABLE_BACKEND_COMPACTION=1` restores the 0.4.x guards as an escape hatch. Legacy `PI_SHELL_ACP_ALLOW_COMPACTION=1` is rejected at spawn intent with a next-action message naming both new knobs. Identity-isolation env (`CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `CODEX_SQLITE_HOME`, `GEMINI_CLI_HOME`, `GEMINI_SYSTEM_MD`) is never touched by either knob. README/AGENTS claims must fit VERIFY/BASELINE/runtime evidence; `./run.sh smoke-compaction-policy` is the 4-step deterministic gate. `LIVE=1 ./run.sh smoke-compaction-policy` adds live steps 03/04/07 against all three ACP backends (Claude, Codex, Gemini) as a uniform probe — NEXT.md tracks the broader context-pressure-continuity investigation that the 0.5.0 release actually rests on. Backend-compact is judged by a dual classifier — text reply ("Context compacted") OR wire-level usage signal (`used=0` from `meter=acpUsageUpdate source=backend`, NOT the `componentSum`/`promptResponse` fallback). Initial outcomes: Codex signals on text; Claude signals on wire; Gemini's ACP path does not honor literal `/compact` as a native compaction command. These are raw probe outcomes, not release claims — the release claim's scope is reconfirmed in NEXT.md after the ACP-standard + three-backend surface investigation lands.
