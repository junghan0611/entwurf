...
11. **Verification must not rewire the operator's own install.** An offline smoke that writes a live `~/.claude` / `~/.gemini` / `~/.pi` path uninstalls the operator as a side effect of "testing". Swap the process HOME to a sandbox first. `check-install-surface` S5 is a static **tripwire** over shell source, not a proof: it catches a literal live path and one hop of aliasing, and it cannot see a path assembled across variables or built inside an embedded heredoc. Read a green S5 as "no obvious destructive line", never as "verification is sandboxed" — the real guarantee is running the offline floor under a swapped HOME, which is still open. LIVE gates are the only surfaces that may drive the real host, and they say so in their name.
12. **A doctor reports what is true, not what it installed.** Read the target's own semantics before calling a host broken. agy matches `mcp(*)` and `mcp(<server>)` against our tool wherever those rules appear, so an operator's broad `allow` already grants `entwurf_v2` — reporting that host as "NOT granted, agy prompts on every call" was a false red about a working surface, and it stood only because the doctor read the same coverage in the deny/ask direction and refused to read it in the allow direction. Installers still take the narrowest rule they need; doctors distinguish **we own this** from **someone else's rule is carrying it** (a note that names the owner) from **it is genuinely broken** (a failure).
...
./run.sh smoke-agy-install-state            # MCP + exact permission ownership + honest inverse (120)
