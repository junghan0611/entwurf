...
CHANGELOG `## NOW — 0.23.1 shipped · #118/#119 세트 닫힘 (main `bdabedc`, 2026-09-20 04:4x)

**끝난 것(전부 영수증은 #118/#119 스레드):** 세트 브랜치 ff-merge `d905a95`(#119 close) → lock `herdr-checkout` 복귀 `3c3a67d` → land(run `35437224014`) → prepare/make `b9a6e4e` = tag `v0.23.1`(run `35455029344`, P5는 observer-amended: aggregate MUST 23/24 + `smoke-codex-fresh-live` 단독 66 assertions, Codex 모델 gpt-5.6-terra; BASELINE 행) → **npm publish 0.23.1(GLG)**, integrity `sha512-T8l/JShW…rndA==` → lock npm 0.23.1 + 게이트 정방향 `194e800` → 출하된 런처 셀 [8a] `086a68e`(설치본 `<active>/node_modules/.bin/entwurf pi --mode rpc`가 시민이 됨, 모델 턴 0) → README 0.23.1 `bdabedc`. (a) 재영수증 main `194e800` PASS 63s, identity npm 0.23.1, cell [1]-[9]+[8a] green. #118 Exit 1-3 + Exit 4 (i)(ii) 충족으로 close.

**GLG가 볼 것 하나(Exit 4 (iii), 이슈 밖):** 날것 PC에서 README 9단계 → Herdr pane 안 `pi --entwurf-control` → `herdr plugin pane open --plugin junghan0611.entwurf --entrypoint status` → 시민이 **ROW**로 그려지는지. 컨테이너는 pane이 없어 못 잰다(unobserved count까지만). 관측되면 #118에 한 줄 코멘트로 남긴다(닫힌 이슈에 코멘트 가능).

**다음 세션의 첫 한 수(별 이슈로 열 것, 큐 상한 10/5):** ROADMAP "0.23.1 뒤 후속" 절 참조 — (1) `smoke-codex-fresh-live` 마지막 단언이 모델 relay 충실도에 걸림(luna는 `$`/`@` sigil 탈락, terra는 보존; VERIFY.md:135-137 기본 모델 문구 갱신 필요) (2) `check-herdr-plugin-build`가 두 lock 종류(npm/herdr-checkout)를 다 읽게 — 컷마다 dd84ac0 정방향/역방향을 뒤집는 현 방식 제거 (3) 잔여 3 gate 이주(`v2-contract`·`v2-production` cross-lane, `v2-surface` `assertRailSemantics`) (4) H2 이식 본체(팬 action/events) (5) npm 소스 swap/torn-swap · package-consumer 재증명.

**규율(계속 유효):** 긴 게이트는 CI 동결 창 1회 · Claude Code로 check:full/release-gate 금지(메모리) · 트레일러 없음 · 컷 동안 체크아웃 실행자 1명 · `entwurf-release` 모드마다 GLG 승인, publish는 GLG.
