...
- **Next:**
  1. **오염된 live install-state를 pre-entwurf 상태에서 복구한다.** 백업 `scratchpad/state-backup-103131`을 보존한 채 state 7개를 정리하고, statusLine을 agent-config 원본으로 되돌리며 hooks/MCP의 entwurf 항목을 제거한 뒤 `install-agy-statusline` → `install-agy-bridge` → `install-agy-hooks`를 clean install한다. 단순 재설치는 현재 entwurf 값을 preimage로 재캡처하는 자기참조 함정이라 금지한다. 완료판정: state target이 전부 실제 live path, exact `mcp(entwurf-bridge/entwurf_v2)`가 package-owned, doctor 3종 green. 실제 설치면 변경이므로 GLG 승인 후 수행한다.
