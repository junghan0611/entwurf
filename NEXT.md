...
- **다음 한 수 (코디네이터):** 새 Opus 호출(인계 절 + 이 절이 브리핑) → H1-4/H1-5 → 검수 → #118 Exit 재대조 → 그때 GLG가 머지·컷을 정한다 → `entwurf-release` land/prepare/make/publish **0.23.1**(모드마다 GLG 승인) → 발행 뒤 새 Opus가 lock 0.23.1 + 플러그인 0.3.0 후속 커밋(`dd84ac0` 패턴) + (a) 재영수증 → GLG 날것 PC 재테스트 = #118 사용자 도착.
