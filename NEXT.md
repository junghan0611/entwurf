...
- **Blocker:** ① live provenance ② agent-config handoff ③ backend drift ④ fresh agy 1.1.2 live 왕복 ⑤ Opus 독립 검수는 닫힘. 남은 것: **⑥ 최종 LIVE release-gate 재실행** — 마지막 17/17 증거는 게이트 수정 이전 트리의 것이다. 제품 표면은 안 바뀌었고 `check-install-surface`는 green이지만, 컷 판정은 재실행 뒤에 쓴다. 그 전에는 0.12.7을 prepare하지 않는다.
