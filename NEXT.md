...
- `scripts/check-entwurf-send-stuck.ts` 제거. Phase B (turn_end) 계약은 폐기했고,
  delivery-ack/jsonl-persist manual smoke 도 send-is-throw 범위를 다시 키우는
  surface 로 판단해 추가 follow-up 에서 삭제.
- `run.sh` / `package.json`: `check-entwurf-stuck` / `check-entwurf-delivery` entry 제거.
...
- ⚠️ `0a43ddf` 의 `--auto-receiver` tmux 자동화 골조 — turn_end 제거 후
  delivery smoke 로 축소하려 했으나, jsonl-persist 검증이 다시 계약을 키우는
  surface 로 드러나 삭제. 필요하면 미래에 별도 ack-only 진단으로 재설계.
...
| **Phase 3 (최종 UX)** | **self-contained plugin** — plugin package 가 bridge runtime 을 직접 품음. `acp-bridge.ts` workspace dep import, child `pi` binary 의존 제거 | `openclaw plugins install @junghanacs/openclaw-pi-shell-acp` 한 줄로 끝. GLG 한 줄 비전 |
...
| **3** | OpenClaw ClawHub 정식 등록 (**리팩토링/정리**) | Phase 2 안정 + ClawHub trust path | `@junghanacs/openclaw-pi-shell-acp` npm publish → ClawHub 등록 → `dangerouslyForceUnsafeInstall` flag 없이 `openclaw plugins install <pkg>`. **새 기능 X — packaging UX 정리.** |
...
└── plugins/
    └── openclaw/             ← NEW. host 어댑터. @junghanacs/openclaw-pi-shell-acp
        ├── package.json
...
| Path | 설명 | 어디서 박혔는가 |
|------|------|---------------|
| **(A) ClawHub 정식 등록** | `trustedSourceLinkedOfficialInstall` — production trust gate | **Phase 3 산출물** (`@junghanacs/openclaw-pi-shell-acp` npm publish + ClawHub 등록) |
...
- **Clean-host 실 설치 검증 (2026-05-18 GLG 결정 · 2026-05-19 공개면 sanitize)**: target = clean Ubuntu 6.8 x86_64 dev host (operator account, `/home/operator`, git 외 toolchain 0) — public docs 에서는 `cleanhost` placeholder 로 지칭, 실제 FQDN/포트/계정은 GLG 외 비공개 (2026-05-19 sanitize 완료, pi.dev 공개면 진입 전 보안 정정). "외부 사용자가 처음 install 할 때" 시나리오로는 이상적 — NixOS dotfile / 우리 home 의존 0.
  - **Toolchain 축**: **Node 24** (`nvm install 24` + `nvm alias default 24`) — `engines.node` 는 `>=22.19.0` minimum 이지만 검증 재현축은 24 명시. pnpm = `corepack prepare pnpm@10.33.0 --activate`.
  - **pi binary**: `npm i -g @earendil-works/pi-coding-agent` (npm latest 0.75.1, bin `pi`).
  - **단계 분리 + 진행 상태 (2026-05-18 KST)**:
    - **Stages 0–3 — clean-host install verified ✅** (Node 24.15.0 / pnpm 10.33.0 / pi 0.75.1 / `pi install git:...` / `run.sh install .` / `pi --list-models pi-shell-acp` 6 models / 4 auth-free 게이트 typecheck + check-mcp 15 + check-models 3-pass + check-backends 136 all green).
    - **Stage 4a — missing-auth boundary verified ✅** (3 backend CLI 부재 상태에서 `Authentication required` / `EPIPE` 정확히 surface — "pi-shell-acp 실패 아님" 라이브 증거).
    - **Stage 4 prep — pi-shell-acp settings + standalone YOLO verified ✅** (`showToolNotifications: true` 박음; `~/.claude/settings.json` permissions wildcard / `~/.codex/config.toml` `approval_policy=never` + `sandbox_mode=danger-full-access` / `~/.gemini/settings.json` `defaultApprovalMode=auto_edit` 모두 thinkpad 의 GLG 환경과 parity 박힘).
...
- **plugin spawn-level tool-notification trace handling (정정 2026-05-18 PM, 코드 정합 2026-05-19)**: 이전 framing (= "publish 전 `showToolNotifications: false` 강제") **폐기**. 이번 top bug reproduce 진행 중 GPT힣 PM 과 재합의 — `false` 강제는 progress 가시성 자체를 죽여서 caller 가 "idle 로 멈춘 듯" 보는 회귀를 만듦. 정공법은 **`true` 유지 + delivery layer 에서 필터 분리**. (a) interactive pi / 디버깅 / 실무 작업 = `true` 권장. (b) OpenClaw / Telegram 의 final answer 채널 오염은 plugin/bot 의 outgoing message layer 에서 `[tool:*]` notice 필터링하거나 progress channel 을 final 채널과 분리해서 해결. (c) publish 기본 문서도 `true` 쪽이 사용자 신뢰에 맞음. **2026-05-19 코드 정합** — `index.ts:621` 의 bridge default 가 `?? false` → `?? true` 로 정정됨 (publish preflight 진입 직전 GPT힣 검토에서 framing-vs-default 불일치 발견 → 정공법 합의대로 정합). `docs/setup-clean-host.md` 의 "why" 박스도 같은 라운드에서 갱신. 진짜 follow-up = 필터 layer 설계 — Oracle workspace `.pi/settings.json` 의 임시 `false` override 는 이전 cover 였고, plugin spawn-level `env=PI_SHELL_ACP_SHOW_TOOL_NOTIFICATIONS=0` 같은 hide 패치도 잠재적 backstep. 다음 라운드에 OpenClaw delivery layer 측에서 final-text 정규화 + progress 분리 작업으로 박을 것 (별도 NEXT 항목 후보, plugin/OpenClaw 측에서 처리).
...
- **pi-tools-bridge MCP async surface (0.7.x or 0.8.0 candidate)**: 외부 MCP host (Claude Code / Codex / Gemini CLI) 에서 `entwurf` 가 sync-only — `mcp/pi-tools-bridge/index.ts` 의 tool description 이 이미 `"(sync mode). ... Async spawn + entwurf_status are not exposed here yet (deferred to a separate design round)"` 으로 honest 하게 말함. 발견 trigger: 2026-05-19 publish-prep 검토 라운드 (분신 GPT-5.4 + GLG 관찰). >30 s 작업 호출 시 부모 turn blocking → ESC 직전 UX 손실. 0.7.0 에서 in-pi tool 의 default 만 `sync` → `async` 로 정합 (`pi-extensions/entwurf.ts`), 외부 MCP surface 는 다음 라운드: (1) MCP tool 에 `mode` 파라미터 노출, (2) `entwurf_status` MCP tool 추가, (3) ACP follow-up notification 채널로 완료 알림 surface 가 가능한지 조사 (Claude Code MCP host 의 notification 채널 지원 여부 의존). 호환성: MCP surface 변경 시에도 sync default 유지하되 explicit `mode=async` 가능하게 (외부 MCP host 의 기존 caller 가 깨지지 않도록).
