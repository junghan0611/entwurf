...
## NOW — 0.23.1 릴리즈 레인 (main `3c3a67d` landed, 2026-09-19 22:xx)

**상태:** 세트 머지(`d905a95`, #119 close) → lock `herdr-checkout` 복귀 `3c3a67d` → `land 0.23.1` 완료(run `35437224014` 4잡 green, body 포함) → `prepare 0.23.1` 진행 중(terra `20260919T135931-24854f`). P5 release-gate 1회차 MUST 23/1/0 — 실패는 `smoke-codex-fresh-live`의 **환경 선행조건**(운영자 Codex app-server 부재), 코드 결함 아님(#118 issuecomment-5742022557). GLG가 Codex TUI를 켠 뒤 terra가 PID를 잡아 P5만 새 scratch에서 재실행한다.

**P5 observer-amended acceptance (GLG 2026-09-20 01:1x):** aggregate 2회차의 나머지 MUST 23/24 및 BEHAVIOR 1/0 위에, luna가 relay에서 `$`·`@` sigil을 탈락시킨 유일한 Codex cell을 `gpt-5.6-terra`로 단독 재실행해 66 assertions exit 0으로 닫았다(#118 issuecomment-5743372882). luna sigil 탈락은 컷 뒤 이슈다.

**실행자 규칙(컷 동안):** 이 체크아웃의 실행자는 **terra 하나**. Fable(코디네이터)·Opus2는 index·파일·커밋을 건드리지 않는다. 기록은 #118 코멘트.

**순서(남은 것):**
1. terra: GLG가 채택한 P5 observer-amended acceptance → P6-P9 → `chore(release): prepare v0.23.1`(이 NEXT 갱신 포함, push 없음) → **`make 0.23.1`**(GLG 19:0x "publish 전까지 진행" 권한: push · exact-SHA CI · tag · GitHub release · candidate tgz 보존) → candidate 경로와 함께 GLG에게 dm(`--as pi/gpt-5.6-terra`).
2. **GLG:** `/entwurf-release publish 0.23.1 <candidate.tgz> latest`.
3. Opus2 `20260919T175543-fe4237`(발행 뒤, GLG 신호): `plugins/herdr/runtime-lock.json` npm `@junghanacs/entwurf@0.23.1` + `npm view @junghanacs/entwurf@0.23.1 dist.integrity` + `check-herdr-plugin-build.ts`를 dd84ac0 정방향(lock+게이트 쌍, 3c3a67d의 역) + README:86-87 복원 + CHANGELOG 줄 → focused(`check-herdr-plugin` · `check-herdr-plugin-build` · `check-herdr-runtime-bootstrap` · `check-gate-manifests`) + `pnpm check` → 커밋 → GLG 승인 뒤 push → `LIVE=1 ./run.sh smoke-herdr-raw-install-live`(main, (a) 재영수증) → **설치된 entwurf bin 경로**로 cell [7]과 같은 0-token birth 셀 추가(#118 Exit 4 (ii)) → #118 코멘트.
4. GLG 날것 PC: README 9단계 + Herdr pane 안 `pi --entwurf-control` → status 팬 ROW(#118 Exit 4 (iii)) → **#118 close**.
5. 컷 뒤 별 이슈(큐 상한 10/5 안에서): 잔여 3 gate(`v2-contract`·`v2-production` cross-lane, `v2-surface` `assertRailSemantics`) · H2 이식 본체 · `check-herdr-plugin-build`가 두 lock 종류를 다 읽게(3c3a67d CHANGELOG (B)) · `scripts/` rename · 검증면 정리.

**Do not:** 로컬 qualification body(CI가 동결 창) · Claude Code로 check:full/release-gate(메모리) · 트레일러 · GLG 승인 없는 publish · 컷 중 둘째 실행자.
