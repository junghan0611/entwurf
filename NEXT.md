...
## NOW — 2026-06-15 KST — 0.11.0 Stage 0 컷 직전: release-gate GREEN(PASS=18) + GPT5.5 검토 GO(수정요청 4 반영) → GLG 컷 결정 대기
...
- **📌 0.11 문서 정합 TODO (GLG 문서검토 2026-06-15 16:10, 아직 미처리):** 옆 GPT 코드 수정이 닫힌 뒤 컷 전 잡을 것. (1) `CHANGELOG.md`의 `PASS=18` 로그는 현재 추가 diff(package/run/preflight/floor 등) 기준으로 stale 가능 → 현 트리 `LIVE=1 release-gate` 재실행 후 로그명 갱신. (2) `README.md`/`VERIFY.md` current MCP surface가 아직 “five tools”라고 말함 — 실제 `pi-tools-bridge`는 7개(`entwurf_send`, `entwurf_v2`, `entwurf_self`, `entwurf_peers`, `entwurf_inbox_read`, `entwurf`, `entwurf_resume`). (3) `VERIFY.md`와 `docs/setup-clean-host.md`의 `smoke-all` 기대 출력이 과거 “Claude+Codex+Gemini” 문구 — 0.11 claude-only floor + codex/gemini on-demand로 정정. (4) `docs/setup-clean-host.md`/public docs pi floor를 현재 `>=0.79.3`와 맞춤. (5) `BASELINE.md` 상단에 0.11.0 release-facing row 추가. (6) `README.md` 초반에 0.11 headline(Stage 0 pi-only `entwurf_v2` substrate + spawn-bg resident lifecycle LIVE + v2-only mode; Claude Code tmux-live는 0.11.1 lane/out-of-scope) 짧게 추가.
