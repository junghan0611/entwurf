...
| **pi 0.79 public export** | `hasProjectTrustInputs`/`ProjectTrustStore`/`getAgentDir`/`VERSION` 모두 index public export 확인 → TS 직접 import 가능(재구현 불필요). **repo는 현재 0.78 → 0.79 bump 필요.** |
...
| trust input 정의 (소스 실측) | `hasProjectTrustInputs`: `.pi`는 **cwd 한정** 체크, `AGENTS.md`/`AGENTS.MD`/`CLAUDE.md`/`CLAUDE.MD`(4종)+`.agents/skills`만 root까지 walk(비대칭). canonicalize=`realpathSync`(없으면 raw 폴백). `CONFIG_DIR_NAME`은 `pkg.piConfig.configDir` override 가능. `ProjectTrustStore.get`은 매 read `proper-lockfile`. **`~/AGENTS.md` 때문에 ~ 아래 전부 trust-input → prefix+`--approve`가 유일 메커니즘.** (이 디테일을 베끼지 말고 pi 함수 직접 import) |
...
5. untrusted controlled launch = **fail-fast** (조용한 `--no-approve` degraded 금지). 근거(소스 확정):
   pi `--no-approve`(`-na`)는 "거부"가 아니라 **project-local 파일을 무시하고 degraded 실행** — 그렇게
   띄운 분신은 그 repo `AGENTS.md`/`.agents/skills`를 못 받아 **담당자 컨텍스트 주입이 깨진다.** trusted만
   `--approve`(project 파일 로드), 아니면 throw.
...
9. **import surface = public root export만** (`@earendil-works/pi-coding-agent`의 `getAgentDir`,
   ...
   tsc 실패 = 공짜 drift 게이트(dev-time). **runtime**: doctor/check가 pi `VERSION >= 0.79.0` 확인 —
...
- **사람이 직접 여는 pi** = global `project_trust` 확장이 preflight 결과를 매핑(안전망):
  ...
  - fail-fast/unknown → interactive(`ctx.hasUI`)면 `{trusted:"undecided"}`(pi 기본 prompt로 defer)
...
- `enrichTaskWithProjectContext` (`pi-extensions/lib/entwurf-core.ts:1101`) → `cwd/AGENTS.md`
- `loadProviderSettings` (`index.ts:642`) → `cwd/.pi/settings.json`
- system-prompt augment (`index.ts:974` 영역) → `cwd/AGENTS.md`

(GPT가 든 `pi-context-augment.ts`는 실재하지 않음 — 위 세 곳이 실제 경로.) 이들을 controlled launch
경로에서 **preflight 뒤로 gate**한다. 사람이 직접 여는 pi의 native trust 흐름은 pi가 순서를 보장하므로
이 가드는 **controlled/entwurf launch 한정**.
...
1. **pi deps 0.79 bump + public-import 가드** — pi-ai/pi-coding-agent/pi-tui devDep 0.79.0, peer
   `>=0.79.0` 검토. `/dist/...` private import 금지 static 가드.
...
4. **TS fact-provider** — `peers`/`who-can`/`preflight`. `meta-session.ts`·mailbox·소켓/tmux probe 재사용,
   기존 `entwurf_peers` MCP로 cross-harness 노출.
5. **상위 entwurf 단일 표면** — preflight 소비 → trusted면 내부 `--approve`로 `pi -p` bg / tmux-live
   dispatch, untrusted면 fail-fast. **preflight를 project-local read보다 먼저**(위 "preflight 정렬 가드").
   primitive 내부 유지.
