...
- **Task 2 — pi provider 이관: 코드 완성(`7240023`, 전체 check green·페블 smoke 독립 재현) / 라이브 이관 미수행 ← 새 오푸스 첫 수.**
  ...
  - **알려진 함정(구 오푸스 실측 인수)**: project remove는 bare bin과 legacy repo path **둘 다** 매칭해야(`_classify`가 해결, 약화 금지) · NixOS엔 /bin/bash 없음 → doctor의 execSync에 shell 옵션 금지(POSIX `command -v`) · smoke는 라이브 노출 bin의 PATH 오염 격리 필수(신규 smoke 작성 시 동일 주의).
