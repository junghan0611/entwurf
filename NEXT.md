...
## ◀ NOW — 세션 #15-2 (2026-06-15, thinkpad, Opus 실무자) — ✅ (A) DONE+커밋 → 🟢 다음 = (B) v2-only mode (0.11.0 마지막 blocker) — 여기부터

> **(B) 한 걸음 (GPT plan, 바로 시작 가능):** `PI_SHELL_ACP_V2_ONLY=1` v2-only mode. 흩뿌리지 말고 **중앙 helper 먼저** → NEW `pi-extensions/lib/entwurf-v2-only.ts`(`isV2OnlyMode(env)` env exact `"1"`만 true / `assertV1EntwurfAllowed(surface,env)` / `v1DisabledMessage(surface)` — 문구는 `PI_SHELL_ACP_V2_ONLY` + `entwurf_v2` + "new sibling create unavailable" 포함, "use entwurf_v2"만 말해 new create 되는 듯 암시 금지). 그 다음 11 entrypoint 초입에 `assertV1EntwurfAllowed` (↓block table 사양): pi-native entwurf(sync/async)·/entwurf·entwurf_resume·entwurf_send / MCP entwurf·entwurf_resume·entwurf_send / **control RPC `spawn_async_resume`(MCP 우회 차단 필수)** / startup `--entwurf-send`(silent skip 금지) / entwurf_v2는 flag-on에서도 동작. gate = pure helper check(exact 1·missing·wrong) + source/static(각 handler 초입 가드 有, v2 block엔 無). **코드 삭제 X·호출 차단만**, 11-scenario v2 replacement는 0.12 lane.
...
> - **다음 한 걸음 (GLG 결정 대기):** ① GLG 커밋 결정 — 이 6파일(`git diff --stat`: entwurf-control 30 / spawn-production 24 / run.sh 45 / check 28 / NEXT 8 + NEW marker·smoke)은 (A)+fix **한 의미 단위** = 독립 커밋 후보 ② **(B) `PI_SHELL_ACP_V2_ONLY=1` v2-only mode** — GPT 권고: 흩뿌리지 말고 **중앙 helper 먼저**(`pi-extensions/lib/entwurf-v2-only.ts` 또는 `entwurf-v1-mode.ts`: `isV2OnlyMode(env)`/`assertV1EntwurfAllowed(surface,env)`/`v1DisabledMessage`) → 11 entrypoint(↓block table)에 초입 가드 → pure helper check + source/static check(특히 control RPC `spawn_async_resume`는 handler source guard 필수). 코드 삭제 X·호출 차단만. ③ CHANGELOG(컷 때) ④ tag. **(A) acceptance blocker = 해소.**
...
- **릴리즈 전 미결 2갭:**
  1. **✅ RESOLVED (세션 #15-2, `c6a478c`) — LIVE acceptance 구멍 닫힘.** spawn real-pi E2E를 `smoke-entwurf-v2-spawn-resume-live`로 LIVE 22 PASS 실견(real pi resume + model turn + lock×1). 닫는 과정에서 production 모순(entwurf-control이 entwurf-tagged resident 거부) 발견·수정. **남은 0.11.0 blocker는 (B) v2-only뿐.** (↓ 옛 분석은 발견 경위 기록으로 보존.)
