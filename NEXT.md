...
> - **게이트(전부 `pnpm check`):** check-entwurf-v2-decider 125(socket-only ff alive→execute/owned→pre-lock bad-target/dormant·indeterminate→reject never spawn), check-entwurf-v2-production 32(record-absent+live→execute, symlink/absent→bad-target, owned→no-lock), check-entwurf-v2-send-fallback 31(socketOnly alive→retry/dead→dormant reject not bad-target). **전체 PASS, 회귀 0.**
> - **테스트로직(LIVE):** `smoke-entwurf-v2-matrix-live`에 **C1b** 추가 — record-less 실 `pi --entwurf-control` resident → socket-only ff control-socket sent + owned→bad-target(no spawn). typecheck/lint/skip-load OK(LIVE 미설정 시 skip).
> - **릴리즈컷 문서:** README 0.11.0 headline(staging tone) + entwurf_v2 socket-only 문단, DELIVERY.md pi-native note, CHANGELOG Fixed bullet, run.sh 4-cell. (BASELINE drop, AGENTS/setup-clean-host skip — GPT sign-off.)
> **★LIVE release-gate 결과 (2026-06-16 11:04, GPT 실행, HEAD 844a302): NOT GREEN `PASS=16 FAIL=2 SKIP=0`.** log `/tmp/pi-tmux-psa-release-gate-011-20260616T110410.log`.
>   - ✅ **A1/C1b 실측 GREEN**: `smoke-entwurf-v2-matrix-live` 17 PASS(C1b record-less socket-only ff→sent, owned→bad-target 전부), `smoke-entwurf-v2-spawn-resume-live` 22 PASS. **recordless-pi blocker는 live로 닫힘.**
>   - ❌ **FAIL 2 (A1 무관, 내 코드 미변경 — resident-garden-guard/sentinel/model-lock/entwurf-control diff 0):**
...
> - **확정 설계 = A1 narrow (GPT5.5 자문 `20260616T094836-a32c30`, Claude lean 일치):** record 없어도 control dir에 gid-shaped non-symlink `*.sock` **presence hint만**(pre-lock connect/probe 금지) 있으면 **socket-only pi endpoint**로 인정 → `entwurf_v2` **fire-and-forget control-send 대상으로만** 승격. "meta identity synthesize" 넓게 X (owned dormant spawn 잘못 열림 방지).
>   - `TargetResolution`에 `socketOnlyPi?: true` source flag 추가. resolveTarget이 record 없을 때 presence hint로 socketOnlyPi 반환. decider가 lock 후 기존 `inspectSocket→mapInspectionToLiveness`로 실제 liveness 판정(pre-lock connect/lstat 금지 원칙 유지).
>   - decider 규칙: `socketOnlyPi && fire-and-forget` → in-domain probe table. `socketOnlyPi && owned-outcome` → **`bad-target` 유지(spawn 절대 안 열림)** — "socket-only endpoint is not an owned citizen". fire-and-forget인데 lock 후 dead/indeterminate → 기존 table reject(`dormant-fire-forget-unsupported`/`indeterminate-no-spawn`), never spawn.
>   - **risk 차단:** synth identity의 cwd를 절대 `process.cwd()`로 안 채움(owned-outcome dead에서 오-spawn 방지) — source flag + owned hard block 필수. dead-control-send fallback은 fire-and-forget pinned라 socketOnlyPi 재-resolve 안전(mailbox로 안 감). listing↔dispatch drift 방지: `socket-discovery.ts`에 shared socket-only candidate/presence helper → `listEntwurfFacts`와 production resolver 공유(garden-id regex/`.sock` suffix/symlink reject/`controlSocketPath`/no pre-lock connect). `scanSocketProbes` 전체는 dispatch에서 호출 금지(pre-lock probe).
>   - **scope:** 이번엔 **alive socket-only pi → fire-and-forget control-send만** 닫는다. **dormant recordless pi → spawn-bg resume는 out-of-scope → 0.11.1**(record 없이 cwd/model/resume authority 없음; JSONL-header authority resume은 별도 설계/A2/Entwurf-core identity layer).
> - **게이트(추가):** ① deterministic: metaRecordExists=false + socketOnly candidate present + alive + fire-and-forget → control-socket execute, lock acquire/release. ② owned-outcome same target → reject, no preflight/spawn. ③ candidate symlink/malformed/absent → bad-target/conflict, no connect. ④ live smoke: recordless live pi control session cell(C1b 추가 또는 C1을 recordless로) — fake-record fixture 갭 닫기.
...
- **검수:** GPT5.5 `20260615T153417-40716d` — **수정요청 후 GO**(코어 6파일 diff 0 / `PI_SHELL_ACP_V2_ONLY` exact "1" / SE 게이트 deliverability·mailbox·self-address PASS 직접확인). 수정요청 4건 = ①CHANGELOG top floor-passed ②SE consumer-visible Fixed bullet ③lane 분리 ④pi >=0.79.1 명시 → 전부 반영(`9048f72`). 직전 검수자 `c34027`은 이제 stale.
- **★퇴근 핸드오프 (ccf877, GLG 지시 2026-06-15):** GLG가 GPT5.5 `40716d`와 직접 마무리. **In-flight = GPT의 mailbox payload fix**(working tree 4파일 uncommitted: `mcp/pi-tools-bridge/src/index.ts`·`pi-extensions/entwurf-control.ts`에 `entwurf_send`/`entwurf_v2` `maxLength:16000` + payload guidance / `scripts/smoke-meta-mailbox.sh` Case D / `CHANGELOG.md` drain-scoped bullet; lint·typecheck·smoke-meta-mailbox·check-pi-tools-bridge-boot PASS). **남은 순서:** ① GPT 4파일 커밋 → ② **0.11 문서 정합 sweep**(아래 docs TODO) → ③ **`LIVE=1 release-gate` 재실행**(코드+게이트 바뀜, PASS=18 재확인 필수) → ④ CHANGELOG 최종 로그/BASELINE 정합 → ⑤ GLG 컷(push + `/prepare-release 0.11.0` → `/make-release`). **새 consumer-visible: maxLength 16000 hard cap.** push/tag/npm publish는 GLG 결정 전 금지.★
- **📌 0.11 문서 정합 sweep (2026-06-16 처리 완료, 단 (1)은 버그로 보류):** (1) ⏸ `LIVE=1 release-gate` 재실행 + 로그명 갱신 = **v2 버그 fix 후로 보류**(위 detour). (2) ✅ `VERIFY.md:5` "five tools"→"seven tools"(entwurf_v2·entwurf_inbox_read 추가). (3) ✅ `VERIFY.md:207/243` smoke-all 기대출력을 실제 `[smoke-all] Claude runtime smoke: ok (codex/gemini live tests dropped — 0.11.0 claude-only floor)`로 정정(README는 이미 정합). (4) ✅ docs에 stale `0.79.x` 표기 0건 — 이미 `2eb34c7`로 정합. (5) ❌ DROP — `BASELINE.md`은 release-gate history가 아니라 harness-identity 인터뷰 질문은행. release-gate evidence는 VERIFY+CHANGELOG에 산다(GPT5.5+Claude 합의). (6) ◑ `README.md` Entwurf 섹션에 `entwurf_v2` additive-verb 문단 + 0.12 forward-pointer(#37) 추가, CHANGELOG line7 transport mis-pair 정정. (상단 top-headline은 선택, 미처리.) **커밋: `docs: …`(아래) — push는 GLG.**
- **GLG가 방금 재확인한 핵심 질문:** “v2가 실제로 spawn/live를 다 하느냐?”에 대한 현재 답은 아래처럼 **정직하게 분리**한다.
  - ✅ **pi live**: `entwurf_v2` fire-and-forget → `control-socket` 전송. 검증: `smoke-entwurf-v2-matrix-live` C1.
  - ✅ **pi background/dormant**: `entwurf_v2` owned-outcome → `spawn-bg` resume → real `pi --entwurf-control` child socket-alive + model turn. 검증: `smoke-entwurf-v2-spawn-resume-live` 22 PASS.
  - ✅/제한 **Claude Code meta-session**: active self-fetch receiver에게 `meta-mailbox` enqueue 가능. 검증: matrix-live C2/C3. **이건 tmux-live가 아님.**
  - ❌ **Claude Code tmux-live**: 현재 v2 production transport로 **미구현**. `tmux-live`는 contract enum에 이름만 있고 decider/runner production path가 실제 실행하지 않음. `claude-code`는 v2 liveness-supported backend가 아니라 `unsupported`로 분류되어 mailbox path만 가능. “창 띄워 서로 대화”식 수동 meta-mailbox와 “tmux live Claude resident”는 다른 것.
- **다음 한 걸음(3분 안):** GLG가 둘 중 하나를 결정한다.
  1. **0.11.0 = Stage 0(pi-only)로 컷 유지** → 이미 release-gate PASS 확인됨 → 원할 때 `/prepare-release 0.11.0` → `/make-release 0.11.0`.
  2. **Claude Code tmux-live까지 v2 직접검수 요구** → 0.11 컷 보류/범위 재선언. **Lane 분리(GLG 방향): Claude Code tmux-live = `0.11.1` / Stage 1 lane; `0.12` = v1 removal/cutover lane(11-scenario replacement·v1 삭제).** 이 경우 현재 0.11 acceptance closed 판단은 “pi-only Stage 0 기준”으로만 유효.
- **Verification floor:** claude-only live floor **실측 완료** — `LIVE=1 ./run.sh release-gate /tmp/psa-release-gate-0.11.0.zVX69T` → **PASS=18 FAIL=0 SKIP=0**. 특히 `(A) smoke-entwurf-v2-spawn-resume-live`가 release-gate 안에서 PASS.
- **Do not touch:** `core.hooksPath`; push/tag/npm publish는 GLG 결정 전 금지. CHANGELOG/tag는 GLG가 “컷 가자”라고 할 때 release skill로.

## RECENT — 닫힌 사실만

- **(A) DONE (`c6a478c` + docs `6f018c7`):** `smoke-entwurf-v2-spawn-resume-live` LIVE 22 PASS. v2 spawn-bg가 real saved Entwurf session을 real `pi --entwurf-control` resident child로 resume하고, socket-alive + user/assistant nonce + lock release×1 + zero-litter까지 확인. 과정에서 `entwurf`-tagged resident name 거부 모순 발견 → sessionId-bound `PI_SHELL_ACP_V2_RESUME_RESIDENT_SESSION_ID` marker로 수정.
- **(B) DONE (`43021b0` + docs `abda09c`):** `PI_SHELL_ACP_V2_ONLY=1`이 10 v1 entrypoint를 side-effect 전 hard-refuse. v1 code 삭제/등록 제거 없음. `entwurf_v2` core flag-clean. `check-entwurf-v2-only` 28 PASS, `pnpm check` PASS.
- **claude-only floor/docs DONE (`a648ddd`, `c4ff212`, `2b801e4`, `6b7195e`):** release-gate live floor는 Claude-only로 재프레이밍. Codex/Gemini는 support에서 빠진 게 아니라 live floor에서 빠진 것(deps/carriers/overlays/on-demand smoke/native v2 Codex child 유지). AGENTS Hard Rule #7은 “capability dignity + honest evidence” 방향으로 조정됨.
- **SE-1/SE-2:** 0.11 deliverability blocker는 CLOSE(CHANGELOG 흡수). residual pi meta-writer/record-only는 SE-3 별도 lane, 컷 blocker 아님.
- **release-gate GREEN (2026-06-15):** `LIVE=1 ./run.sh release-gate /tmp/psa-release-gate-0.11.0.zVX69T` → **PASS=18 FAIL=0 SKIP=0**, log `/tmp/pi-shell-acp-release-gate-0.11.0-20260615T152058.log`. Necessary condition met; cut itself remains GLG decision.
