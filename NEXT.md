...
- **plugin spawn-level tool-notification trace handling (정정 2026-05-18 PM)**: 이전 framing (= "publish 전 `showToolNotifications: false` 강제") **폐기**. 이번 top bug reproduce 진행 중 GPT힣 PM 과 재합의 — `false` 강제는 progress 가시성 자체를 죽여서 caller 가 "idle 로 멈춘 듯" 보는 회귀를 만듦. 정공법은 **`true` 유지 + delivery layer 에서 필터 분리**. (a) interactive pi / 디버깅 / 실무 작업 = `true` 권장. (b) OpenClaw / Telegram 의 final answer 채널 오염은 plugin/bot 의 outgoing message layer 에서 `[tool:*]` notice 필터링하거나 progress channel 을 final 채널과 분리해서 해결. (c) publish 기본 문서도 `true` 쪽이 사용자 신뢰에 맞음. 진짜 follow-up = 필터 layer 설계 — Oracle workspace `.pi/settings.json` 의 임시 `false` override 는 이전 cover 였고, plugin spawn-level `env=PI_SHELL_ACP_SHOW_TOOL_NOTIFICATIONS=0` 같은 hide 패치도 잠재적 backstep. 다음 라운드에 OpenClaw delivery layer 측에서 final-text 정규화 + progress 분리 작업으로 박을 것 (별도 NEXT 항목 후보, plugin/OpenClaw 측에서 처리).
