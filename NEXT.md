...
- **방향 재고정 (GLG 2026-09-19 18:0x):** **아직 머지도 릴리즈도 안 한다. #119가 끝났으니 같은 브랜치에서 #118로 넘어간다.** CI는 기다리지 않는다(터질 수 있고, 릴리즈 SHA에서 다시 돈다). 점심 결정 원문: "#119 테스트를 행동 옆에 두고 자동 발견·실행, 기존 검사를 계약 보존하며 정돈 → #118 그 구조 위에서 Herdr의 실제 **설치·사용** 경로 검증".
- **H1-4/H1-5 착지 (Opus2 `20260919T175543-fe4237`, 2026-09-19 18:09):** 커밋 `01e1491` 한 개 — raw-install smoke에 cell [6] 설치 런타임 측정(compiled entry `mcp/entwurf-bridge/dist/mcp/entwurf-bridge/src/index.js`·bins 3·`check-bridge` exit 0·VERBS 7) · [7] 사용 경로(`pi --entwurf-control --mode rpc` + `get_state` 한 번 → V3 record·socket·peers alive, 모델 턴 0) · [8] status 팬(서버 없음 → `herdr-agent-list-failed` red; headless server → "not shown … unobserved"; 팬 ROW는 herdr pane 필요 → **호스트 잔여**) · [9] deactivate roundtrip(npm 소스, 재설치 후 `["pi","claude-code"]`). 영수증: `LIVE=1 ./run.sh smoke-herdr-raw-install-live --ref set/119-verify-herdr` PASS 65s(2회차; 1회차는 정규식 `[a-z_]`가 `entwurf_v2`를 `entwurf_v`로 찍어 고침) · `pnpm check` 57s · `check-release-gate-outcomes` ok. #118 본문 표 + 코멘트(issuecomment-5740675991) 갱신됨. **검수:** terra `20260919T135931-24854f` read-only 진행 중 → 결과는 `/tmp/entwurf-118-h1-review-terra.md`. terra 결과 Blocker 0 / Defect 2 / Observation 1(issuecomment-5740696589) → **A5 `d0bf334`**(cell [6] 기대 verb 집합 JSON 동등 · [9] absent baseline을 계약상 빈 껍데기 `{"packages":[]}`로 좁힘, 근거 `register-pi-package.py:22,307-323` remove()는 삭제가 아니라 필터 · [8] `grep -qx`로 "1 citizen(s) not shown … (unobserved)." 전체 줄). 영수증: LIVE smoke PASS 65s @ `1685d07` resolved, `pnpm check` 58s, bash -n 바깥+추출 러너. **#118 Exit 1·2·3 전부 충족**(본문 갱신됨). 남은 결정 = GLG 머지·0.23.1 컷. Opus2 대기(워킹트리 클린).
- **#118 남은 것 (설치는 됐고 사용·재증명이 남았다):** (H1-4) 플러그인 설치 **뒤의 사용 경로** — 그 런타임으로 `pi --entwurf-control`이 시민이 되고(record·socket), `entwurf_peers`가 보고, status 팬(`lib/status.mjs`)이 그 시민을 보여주는지, 모델 턴 없이; (H1-5) VERIFY.md:61 npm 재증명 표의 ✗ 행 중 컷 없이 닫을 수 있는 것 — 설치 런타임의 compiled entry·bins 3·실제 `check-bridge`, `herdr-plugin-deactivate` roundtrip — 을 raw-install smoke의 cell로. lock 핀·consumer proof는 0.23.1 뒤.
- **세트 검수 2회 + astra 자문 (2026-09-19 18:5x~19:0x):** glm 세트 검수 Blocker 0/Defect 1(README (b) 인용)/Obs 3 → A6 `c5d187e`; grok 세트 검수 Blocker 0/Defect 0/Obs 6 → A7(smoke `:305-307` 배선 JSON 구조 동등 · `:340,:695` MCP owner 키 리터럴 · README 팬 ROW 호스트 절차 한 줄) Opus2 진행 중; CI 동결 영수증 #119 게시(`1a8042a` check:full · `b9058fd` body 52분). **astra 자문(#118 issuecomment-5740907735):** ① #118은 Exit 4(사용자 수용: 핀 뒤 (a) 재실행 · 설치된 bin 경로의 0-token birth · 날것 PC 팬 ROW)까지 열어둔다, #119는 머지 SHA close. ② **컷 함정:** root 0.23.1 + lock npm 0.23.0 창은 `check-herdr-runtime-bootstrap` 8a `npmCoherent`(check:hermetic)가 거절 → prepare check:full red. 0.23.0 때도 선행 lock은 `herdr-checkout`(`dd84ac0`). **순서: 머지 → lock `herdr-checkout` 복귀 커밋 → land/prepare/make/publish 0.23.1 → lock npm 0.23.1 + 플러그인 0.3.0 커밋 → Exit 4.** 두 이슈 본문에 반영됨.
- **다음 한 수 (코디네이터):** ~~새 Opus 호출 → H1-4/H1-5 → 검수 → #118 Exit 재대조~~(전부 끝, `d0bf334`) → **여기서 GLG가 머지·컷을 정한다** → `entwurf-release` land/prepare/make/publish **0.23.1**(모드마다 GLG 승인) → 발행 뒤 새 Opus가 lock 0.23.1 + 플러그인 0.3.0 후속 커밋(`dd84ac0` 패턴) + (a) 재영수증 → GLG 날것 PC 재테스트 = #118 사용자 도착.
