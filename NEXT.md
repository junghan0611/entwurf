...
### 실행 순서 — **A 완료** → C prototype(무승인) ∥ B+B2 live evidence(승인) → exec form 판정 → artifact harness → clean-host acceptance → #49 C/E
...
- **다음 Opus 실무자 — 무승인 NOW는 산출물 C 하나다.** Node **24+** `check-install-container` prototype: host가 한 번 만든 candidate `.tgz`만 read-only로 넘긴다. foreign cwd, `NODE_PATH=''`, checkout/repo `node_modules` 접근 0, non-root + 격리 npm prefix를 포함한다. tarball 이름은 `package.json` version에서 파생한다. 심은 결함으로 RED를 한 번 보여 vacuous pass가 아님을 증명한다. hook form과 무관하므로 exec 판정을 기다리지 않는다. repo에 최종 gate로 커밋하지 말고 prototype 명령·로그·artifact digest를 산출물로 남긴다.
...
- **Node runtime 결정(GLG, 2026-07-21): Node 24+ 단일 지원축.** 이 리포는 dev-clone의 native TypeScript strip/ESM 경계와 설치판 compiled-JS 경계를 Node 24에서 설계·검증해 왔고, 높은 런타임을 의도적으로 택한다. 현재 `engines.node >=22.6.0`, setup/meta installer/doctor preflight, start.sh 주석, clean-host 문서는 stale 선언이다. 하네스 구현과 같은 변경에서 `>=24` 계약·모든 preflight·docs·version-coherence gate를 함께 움직인다. GitHub Actions가 Node 24라는 사실만으로 runtime floor를 증명했다고 말하지 않는다 — `node:24-*` package-consumer가 실제 candidate tarball을 설치·실행하는 것이 증거다.
- **실무자 금지/운영자 결정:** 운영자 pnpm 캐시 갱신은 첫 작업도 blocker도 아닌 별도 operator action이며 GLG 승인 대기다. `0.12.7-1`은 이미 출하된 `0.12.7`보다 낮은 체크포인트 이름이므로 publish하지 않는다. 권고안 `0.12.8-repair.0 --tag repair`, pi peer 범위, version/dist-tag, 운영자 캐시, 새 Claude 세션은 모두 GLG 결정/승인 사항이다. 다음 실무자가 임의로 건드리지 않는다. C·E도 이 하네스 축이 닫힐 때까지 열지 않는다.
