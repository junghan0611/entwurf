...
| **1** | OpenClaw 프리릴리즈 + Oracle 검증 | Phase 0 완료 (✅) | `plugins/openclaw/` scaffold (monorepo lite). 수동설치 (git-local / tarball). Oracle 에서 daily-use. **npmjs publish 안 함.** |
| **2** | pi.dev 패키징 준비 | Phase 1 의 Oracle 검증 안정화 | `packages.md` 룰 기준 — `pi.extensions` manifest, `files` allowlist, `typebox` peer dep, 4-axis tarball verification. #15 stabilization 병행. **pi.dev 등재 push 는 GLG 직접 결정.** |
| **3** | OpenClaw ClawHub 정식 등록 | Phase 2 안정 + ClawHub trust path | `@junghan0611/openclaw-pi-shell-acp` npm publish → ClawHub 등록 → `dangerouslyForceUnsafeInstall` flag 없이 `openclaw plugins install <pkg>` |
...
## Phase 1 — OpenClaw 프리릴리즈 + Oracle 검증 (현재 priority)

> 결정: **OpenClaw plugin 을 우리 리포에 품는다 — 프리릴리즈로.**
> 수동설치 가능 (git-local / tarball). **npmjs publish 안 함.** Oracle 에서 daily-use 로 검증.
> 동작 잘 되는 것부터 확보 → 검증된 surface 를 Phase 2 에서 packaging.
...
> #15 stabilization 통합 — no-feature hardening.
...
- **Telegram delivery bridge 정식화 (Phase 1.4)**: Phase 1.8 응급 다리로 child pi final text → synthetic OpenClaw `message` toolCall 변환을 stub 에 넣음 (`pi-shell-acp-message-*`, toolResult 후 즉시 `end_turn`). `exit` 후 `close` 지연/defunct 가능성 때문에 stub 에 child lifecycle DIAG + exit fallback finalize 도 추가. 정식 작업에서는 OpenClaw `context.tools`/provider tool surface 를 pi-shell-acp transport 에 연결하는 일반 tool bridge 로 승격. 지금 패치는 Telegram/message-tool-only path 를 뚫기 위한 prerelease shim.
