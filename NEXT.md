...
**Progress (2026-05-29) — entry point for the implementation session.**
- ✅ **DONE (gate hardening + #26):** Step 1 — 1b (fold `check-model-lock` + `verify-transcript-poison` into `pnpm check`); 1c (`check-dep-versions` 6→12 assertions incl. the three pi devDeps + `check-pack-install` peer pin); 1e (`./run.sh release-gate` skeleton, `-xt` fail-closed stub); 1f (prepublishOnly already static+pack) — commits `12d55eb`, `6592c5f`. Step 2b #26 no-auth sentinel + `check-auth-boundary` guard — commit `ef56394`, **verified live** (release-gate `PASS 7→12`, 5 gates recovered, `No API key` count 0).
- ✅ **DONE (sentinel — full matrix green):** `scripts/sentinel-runner.sh`, `pi-extensions/entwurf.ts`, `pi-extensions/entwurf-control.ts`, `pi-extensions/lib/entwurf-async.ts`.
  - **cell 4/5/6 `[R2]` = test-setup gap, CONFIRMED.** Added `--entwurf-control` to the acp-claude/acp-codex parent spawns only (native parents untouched — they use the in-process callback). `./run.sh sentinel 4,5,6` → **3/3 PASS**. No production code change needed; ACP-parent async resume works once the control socket exists.
  - **arg footgun fixed:** `main()` now merges all positional args (`"${*:-all}"`), so both `sentinel 4 5 6` and `sentinel 4,5,6` resolve — it previously read only `$1` and silently ran cell 4 alone.
  - **cell 2 `[R1]` = real best-effort-delivery weakness, FIXED.** New shared helper `makeBestEffortDeliverCompletion` (`lib/entwurf-async.ts`): catches ONLY the stale-ctx error (`"is stale after session replacement or reload"`), drops with a stderr diagnostic, never re-delivers to another live ctx; any other `sendMessage` error re-throws (crash-loud). `./run.sh sentinel 2` → **PASS**.
  - **🎁 bug found during the gate: identical stale-ctx crash path in `entwurf-control.ts:993`** (the RPC-driven `spawn_async_resume` delivery for ACP parents) — same race, was unguarded. Folded into the same shared helper so both deliverers (native `entwurf_resume` tool + entwurf-control RPC) share one guard.
  - `./run.sh sentinel` (all 6) → **6/6 PASS** (artifact `/tmp/sentinel-20260529-131127.json`). `pnpm typecheck` + `pnpm lint` green.
- ✅ **DONE (`-xt` ACP-backend exclude-tools policy — fail-fast, decided GLG+GPT 2026-05-29):** `index.ts` `assertExcludeToolsHonored` rejects `-xt <built-in>` (read/bash/edit/write) up front on every backend — Claude (vs `providerSettings.tools`), Codex/Gemini (native shell+file always present) — because the backend CLI still exposes the capability (declared≠actual). Extension tools (entwurf, entwurf_send) stay freely excludable (pi-side, never reach the backend). Surfaces as a clean stream error (stopReason=error, NO backend launch, 0 tokens). `run.sh xt-tool-surface` is now a real live gate (replaced the fail-closed stub); release-gate step 4 calls it. **Live-verified 3-backend + positive control:** claude/codex `-xt bash` rejected, gemini `-xt write` rejected, `-xt entwurf` honored. (Chose fail-fast over per-backend mapping: each backend names/denies tools differently → mapping is high release risk; thin honest stance for 0.8.0.)
- ✅ **DONE (pre-dep baseline release-gate + 🎁 gift bug):** ran the full `./run.sh release-gate` from a **scratch cwd** (`/tmp/pi-shell-acp-release-gate-20260529`) so test sessions don't pollute the repo session dir (88 prior repo sessions archived 2026-05-29). Result 13/14 PASS; the one FAIL was a **test bug, not a product bug**: `session-messaging` mcp→native / mcp→ACP cases. Root cause — the smoke truncated the `entwurf_send` response to `text[:200]` then grepped for "delivered", but the send-side preview block (`[entwurf sent →]` + to/from/mode/preview, since 0.4.14) puts "✓ delivered" at the end; whether it lands within 200 chars depends on the **sender cwd path length** (`abbreviateHomeMcp` shortens $HOME paths but not `/tmp` scratch). Repo cwd (abbreviated `~/…`) fit; scratch cwd (`/tmp/…`, ~17 chars longer) pushed "delivered" past the window → false FAIL. Fixed `scripts/session-messaging-smoke.sh` to compute the delivered flag on the **full** response text (truncated slice kept only as artifact evidence). Re-ran from scratch cwd → **4/4 PASS**. So the full gate is now 14/14.
  - **Testing discipline:** always run live gates / sentinel / smoke from a **scratch cwd**, not the repo — cwd determines the pi session dir, and repo-cwd runs litter `~/.pi/agent/sessions/--home-…-pi-shell-acp--`. Recommended scratch: `/tmp/pi-shell-acp-release-gate-*`. Do NOT pass `--allow-skip-gemini` at release (gemini is present).
- ✅ **DONE (test-infra cwd/async hardening):** commit `6e899d2` preserves scratch cwd for live gates (`smoke-async-resume`, `check-bridge`, `check-native-async`; tmux spawns pinned with `-c "$PWD"`) and hardens Claude-family async-resume prompt variance with bounded retry + explicit `MODEL_ARG_OR_ENVELOPE_MISMATCH` classification. Verified from scratch: repo session dir delta `+0`, scratch session dir `+7`, `smoke-async-resume` `6/6 PASS` including Claude async ack.
- ✅ **DONE (Step 2 dependency bump):** commit `9f49121` aligns `@earendil-works/pi-{ai,coding-agent,tui}` to `0.77.0`, `claude-agent-acp` to `0.38.0`, and `codex-acp` to `0.15.0`; README and `run.sh` pins updated, `pnpm-lock.yaml` regenerated. Pre-commit `pnpm check` green.
- ▶ **NEXT (new session, in order):** (1) run/record scratch full `./run.sh release-gate /tmp/pi-shell-acp-release-gate-20260529` after the dep bump if not already recorded green; (2) Step 3 **retire Opus 4.7 and replace all live/test/docs surfaces with Opus 4.8 only** — no 4.7 test target remains; add/adjust Claude-family async evidence with `claude-opus-4-8` after the 4.8 surface exists; (3) Step 4 docs; (4) Step 5 cut. **No `--allow-skip-gemini`; all live gates from scratch cwd. GLG closes final 0.8.0 authorization after a green full release-gate.**
...
**Post-#26 release-gate observation (2026-05-29, log `/tmp/release-gate-postfix-20260529-112901.log`):** #26 no-auth sentinel fix (`ef56394`) removed every `No API key found for pi-shell-acp` failure (`grep count = 0`) and improved baseline `PASS=7 FAIL=7` → `PASS=12 FAIL=2`. `check-bridge`, `smoke-async-resume`, `smoke-entwurf-resume`, `smoke-compaction-policy`, and `verify-resume` flipped to PASS. Remaining blockers are: (1) expected `xt-tool-surface` fail-closed stub, (2) `sentinel` real regression. Sentinel post-fix details: cells 1 and 3 PASS; cell 2 fails `[R1]` with `This extension ctx is stale after session replacement or reload` in async completion delivery; cells 4/5/6 fail `[R2]` with no turns increase / missing parent control socket for ACP parent async-resume delivery. This is separate from #26 and should be investigated before dependency bump so pi 0.77 async-resume compatibility is understood.

**Sentinel root-cause analysis (2026-05-29, code + pi-mono 0.77.0 source). Two distinct, separable causes — NOT one bug. ✅ BOTH RESOLVED 2026-05-29 (see Progress block above) — kept here for trace.**

- **cell 2 `[R1]` — pi-shell-acp async-delivery weakness (REAL, fix in OUR code).** `entwurf.ts:853` `deliverCompletion = (msg) => pi.sendMessage(...)` captures the parent `pi` ctx; the async completion fires later from `entwurf-async.ts:351` `proc.on("close")`. If the parent session was disposed/replaced by then, pi throws `ctx is stale`. This is NOT a new 0.77 guard (stale-ctx guard exists since ~v0.70); 0.77.0 `5b31ffd7 Abort session work during dispose` made teardown stricter, so the latent race surfaces. **0.77 is correct — we must deliver to the parent best-effort:** re-acquire a live ctx at delivery time, or try/catch the stale case and degrade gracefully (the entwurf already completed; only the notification delivery fails — it must not crash the parent to rc=1). Timing race explains why cells 1/3 pass and cell 2 (slower claude target) throws.
- **cell 4/5/6 `[R2]` — sentinel setup gap (likely TEST, confirm first).** ACP parents are spawned WITHOUT `--entwurf-control` (`sentinel-runner.sh:147-159`), so no control socket exists; ACP-parent async resume routes through the MCP `spawn_async_resume` RPC which needs that socket (`mcp/pi-tools-bridge/src/index.ts:117` throws "No pi control socket"). Native parents use the in-process callback and don't need it (cells 1/3 pass). GLG's real-use alias DOES pass `--entwurf-control`, so sentinel under-reproduces production. **First step: add `--entwurf-control` to the acp-claude/acp-codex parent spawns and re-run sentinel** — if 4/5/6 flip to PASS it's a pure test-setup fix; if not, ACP-parent async resume is genuinely unsupported and needs a code decision.

**pi-mono 0.77.0 contract changes behind all 0.8.0 work** (verified in `~/repos/3rd/pi/pi-mono`, HEAD `8322745e`): `3e9f7174` make config env references explicit → #26 surface; `9380d5f2` add exclude-tools option → `-xt`; `5b31ffd7` abort session work during dispose → sentinel cell 2 surface.
...
- **ACP backend `exclude-tools` policy — a RELEASE-BLOCKING FIX, not just a smoke (code-level confirmed, GPT-5.5 4th review; scoped to all backends per 5th review).** Name it the **pi-shell-acp ACP backend exclude-tools policy**, NOT a "Claude fix" — Codex/Gemini have differently-named native tools and the same truthfulness problem applies. The divergence is real in code:
  ...
  - **0.8.0 decision (DECIDED GLG+GPT 2026-05-29 → option b, fail-fast; IMPLEMENTED):** ~~(a) per-backend exclusion mapping~~ rejected — each backend names/denies tools differently (Claude `tools`/`disallowedTools`, Codex `exec_command/apply_patch`, Gemini `run_shell_command/…`), so mapping is high release risk. Chose **(b) fail-fast**: pi-shell-acp rejects `-xt <built-in>` up front whenever it cannot be honestly reflected in the backend surface (`index.ts` `assertExcludeToolsHonored`). Live gate `run.sh xt-tool-surface` asserts per-backend fail-fast + extension-exclusion exemption.
...
**Live repro (2026-05-29, parent pi session):** this is now confirmed as an actual runtime blocker, not cosmetic warning cleanup. `entwurf(cwd=/home/junghan, provider=pi-shell-acp, model=gemini-3.1-pro-preview, mode=async)` failed before backend launch with `No API key found for pi-shell-acp` plus the `ANTHROPIC_API_KEY` legacy-env deprecation warning. Task `77714cca` produced no session file. The failure happens at pi provider auth preflight, so the request never reaches the Gemini CLI child even though backend auth should belong there. Issue #26 has the repro comment: https://github.com/junghan0611/pi-shell-acp/issues/26#issuecomment-4569816236
...
- **Verification (folded into the Step-1 release gate):**
  ...
  - Live: with `ANTHROPIC_API_KEY` unset and no `pi-shell-acp` auth entry, `entwurf(cwd=/home/junghan, provider=pi-shell-acp, model=gemini-3.1-pro-preview)` reaches backend launch / succeeds instead of failing at pi provider auth preflight.
...
**External MCP PATH / `pi` ENOENT (reported 2026-05-29 from Doom Emacs Claude Code):** `pi-tools-bridge` started, but `entwurf` spawn failed because the MCP server launch environment lacked `~/.local/share/pnpm`, so child spawn of `pi` hit ENOENT. Direct shell `pi --session ...` worked because the interactive shell PATH included the pnpm bin dir. This is not #26 and not model identity splice; it is an external-MCP host environment issue. Record/fix in docs and maybe code:
- Immediate workaround: add a full `PATH` including `/home/junghan/.local/share/pnpm` to the Claude Code MCP server env, or set `PI_TOOLS_BRIDGE_ENV_FILE` to a file sourced by `mcp/pi-tools-bridge/start.sh` that exports PATH, or symlink `~/.local/share/pnpm/pi` into a directory the host already exposes (e.g. `~/.local/bin/pi`).
- Durable repo option: add an env override such as `PI_ENTWURF_PI_COMMAND=/absolute/path/to/pi` for local MCP spawn paths, then document it for external hosts with sanitized PATH. Keep public-package neutrality — do not bake personal pnpm paths into `start.sh`.
- Operational workaround observed from Claude Code: when MCP `entwurf` spawn is blocked by PATH, run direct `pi --session <existing-entwurf-jsonl> ...` from the host shell in the background and write the prompt to a temp file. This preserves the saved session context and bypasses the MCP server env, but it is a manual external-host workaround — not a replacement for fixing MCP PATH / spawn configuration.
- README currently says external MCP prerequisite is "`pi` on PATH"; strengthen the Claude Code example to mention that the MCP server may not inherit the interactive shell PATH and should pass PATH or `PI_TOOLS_BRIDGE_ENV_FILE` explicitly. Also document the direct `pi --session` background workaround as an emergency/manual path for long reviews.
