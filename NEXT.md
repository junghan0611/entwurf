...
### Current mode — `/gnew` landed (hardening cycle 3); prior 17/0 INVALIDATED → GLG commit → re-run authoritative release-gate → publish
...
**Hardening cycle 2 — session-replacement identity invariant (2026-06-04, GLG hit it live: `/new` killed the process).** The 0.9.0 resident guard fires at `session_start` for EVERY reason, but in-process session replacement (`/new`, `/fork`, `/clone`, RPC `new_session`/`switch_session`/`fork`/`clone`, keybindings, `ctx.newSession`) mints a pi `uuidv7` — no `--session-id` reaches an in-process switch, and `SessionBeforeSwitchResult`/`ForkResult` carry only `{ cancel }` (id is launch-fixed, cannot be injected). So a routine `/new` reached the hard guard and `process.exit(1)` the WHOLE pi process. **This is a 0.9.0 self-inflicted cut blocker** (the guard created it). Code-verified that pi routes every in-process path through `AgentSessionRuntime.{newSession,switchSession,fork}` → `emitBeforeSwitch`/`emitBeforeFork`, which honor `cancel` — so a pre-event guard catches all entry points at once. Fix (`entwurf-control.ts`): under `--entwurf-control`,
- `session_before_switch` reason `"new"` → cancel + guide to the garden launcher;
- `session_before_switch` reason `"resume"` → read `targetSessionFile` header id; cancel if non-garden (friendly pre-cancel instead of the hard-exit; garden target passes; unreadable header falls through to the `session_start` backstop);
- `session_before_fork` → always cancel + guide;
- `session_start` hard guard retained as the last-resort backstop.

Verified live via GPT's zero-token RPC repro pattern: `new_session` + `clone` both return `{cancelled:true}`, process survives (exit 0), `get_state` keeps the original garden sessionId, the "Non-garden session id" hard guard never fires. Locked into `smoke-resident-garden-guard` as a new **REPLACEMENT** section (0 tokens, RPC), now in `release-gate` (PASS=15: negative 6 + replacement 5 + positive 4). **`resume` pre-cancel is NOT yet live-proven** (needs a synthetic legacy-uuid session file) — backstop guarantees safety; add a live/deterministic resume-into-uuid gate next.

**Hardening cycle 3 — `/gnew`: garden-native in-process new session (2026-06-04, GLG/GPT/Claude triad).** Blocking `/new` left a UX hole: no in-process way to start a *new* garden session in the same terminal. `ctx.newSession({setup})` is NOT the answer — verified in pi 0.78 runtime (`agent-session-runtime.js:143`): `newSession` runs `SessionManager.create()` (mints a uuid) and fires `session_start` BEFORE the `setup` callback, so the backend/bridge identity binds to the uuid first and a later header re-stamp only tears it. The clean path (GPT) is **a precreated empty garden file + `ctx.switchSession(file)`**: `switchSession`→`SessionManager.open()` reads the garden header id BEFORE `session_start`, so no uuid moment ever exists. And it IS headless-testable — `session.prompt("/gnew")` intercepts the slash and runs the registered command (whose `ctx` has `switchSession`) at 0 tokens, BEFORE any model turn (`agent-session.js:707`). Landed:
- **`createGardenSessionFile()`** in `entwurf-core.ts` (the fail-closed writer): absolute-cwd + absolute-sessionDir guards, `wx` no-overwrite + collision header-scan + full read-back (type/version/id/cwd/timestamp) → unlink+throw on ANY mismatch, so `setSessionFile`'s silent uuid-remint-on-bad-header trap can never fire. `removeUnadoptedGardenSessionFile()` cleans the orphan only on a cancelled/failed switch (a *successful* switch's empty session is kept — it's a legitimate fresh session, same as a launcher session quit before its first turn).
- **`/gnew` + `/garden-new`** in `entwurf-control.ts`: `waitForIdle` → writer → `switchSession` (orphan-unlink on cancel/throw; old `ctx` untouched after a successful switch). Builtin `/new` stays blocked.
- **Tests.** T1 deterministic writer in `check-entwurf-session-identity` (158 assertions: header shape, wx, collision-no-uuid-fallback, read-back incl. timestamp, absolute-sessionDir guard, guarded cleanup). T2 0-token `/gnew` E2E + T3 1-turn backend identity in `smoke-resident-garden-guard` GNEW section, driven by `scripts/gnew-rpc-drive.ts` (run via `node --experimental-strip-types`; inside the `scripts/tsconfig.json` typecheck fence — no `.mjs` opt-out). Sequential RPC: `prompt "/gnew"` waits before `get_state`. **Live-proven: T2 24/0 0-token** (new garden id ≠ old, 0 tokens, no-rewrite-mint, no conversation messages, `control` name set, socket rebound + old dropped + no uuid leak; T3 under `SMOKE_RGG_POSITIVE=1`: backend `entwurf_self` reports the NEW garden id → `PI_SESSION_ID` propagated through the in-process switch to the MCP child, never the pre-switch id). Full `pnpm check` green.
- **Known minor:** `/gnew` persists the header+metadata file immediately (switchSession needs it to exist), so repeated `/gnew` without turns leaves empty session files — more than the launcher (which defers the file to first turn). Acceptable for 0.9.0; a cross-cutting empty-session GC (applies to the launcher too) is the follow-up, not a `/gnew` defect.
