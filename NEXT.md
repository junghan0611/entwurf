...
- **Active install SSOT:**
  ...
  - statusline: `~/.gemini/antigravity-cli/settings.json` → `statusLine.command=entwurf-agy-statusline`
  - MCP bridge currently: `~/.gemini/antigravity-cli/mcp_config.json` → `mcpServers.entwurf-bridge`
- **Important correction:** earlier `~/.gemini/antigravity-cli/hooks.json` single-file conclusion was stale. Live agy actually loaded plugin hooks (`jsonhook.go: Loaded hooks.json from ~/.gemini/config/plugins/...`). Installer now uses plugin SSOT and cleans/guards legacy surfaces.
- **Next:** fix/decide the **MCP tool injection surface**, not birth. Agy can birth itself, but when asked “entwurf로 20260709T182857-36b3e9에 인사 보내봐”, the model did not directly see/use `entwurf_v2`; it read files and ran `run.sh doctor-agy-bridge`. Logs show permission prompts (`persistGrants=[mcp(entwurf-bridge/*)]`) and no shared config permissions (`~/.gemini/config/config.json` absent permissions). Decide whether MCP must move from global `~/.gemini/antigravity-cli/mcp_config.json` into a plugin `~/.gemini/config/plugins/entwurf-bridge/mcp_config.json` + plugin manifest, and wire default permission grants if safe.
- **Blocker:** MCP tool injection/permission is not closed. Do not call #46 complete until a fresh agy session can directly call `entwurf_v2` without reading runbooks or shelling out.
