...
| **step 4 (진행 중)** | TS fact-provider (`peers`/`who-can`/`preflight`, facts only) — `meta-session.ts`+mailbox+`socket-probe.ts`(3값, F3서 선추출 완료) 재사용, 기존 `entwurf_peers` MCP 노출. **slice 1 ✅ `2ed4603`**(순수 `PeerFact`+`resolvePeerFact`, `check-entwurf-facts` 51). **slice 2 ✅ 순수 코어**(2026-06-11, 설계 ↓ "step 4 slice 2 설계 동결" 섹션) = `resolveFactList(identities, socketProbes) → {peers: PeerFact[], socketOnly: SocketOnlyFact[]}`, dedup=gardenId, dormant probe(absent→dead, unprobed→throw)·F3 indeterminate 보존·non-pi+socket fail-loud, `check-entwurf-facts` **82**. IO 배선은 slice 3. **slice 3 ✅**(`socket-discovery.ts` `scanSocketProbes` = (dir 소켓)∪(in-domain 시민 canonical path) 3값 probe, dormant no-file→dead/stall→indeterminate, dir hygiene/dedup, deps 주입, `check-socket-discovery` **18** + e2e→resolveFactList). **slice 3 잔여(후속, 범위 분리): get_info enrich(cwd/model/idle) 배선** — SocketProbe enrich는 현재 null(probe-only, 정직); 기존 `getLiveSessions`/`WithInfo`의 이 lib 전환은 additive 교리상 후순위(레거시 listing 무변경). 남은 slice 4(MCP 노출+who-can/preflight) + pi session_start meta-record writer(별도) | fact-provider 게이트 | contract + 버킷 B |
| **step 5** | `entwurf_v2` 단일 표면(레거시 공존) — preflight+fact-provider 소비 → liveness로 resume/send call-time 계산, trusted→내부 `--approve` `pi -p` bg / tmux-live, untrusted→fail-fast. **진입 전 BLOCKER = F-mailbox**(↑ "F-mailbox" 블록: v2에 mailbox-deliverability 축 추가) | dispatch 게이트 + live smoke | step 4 + **F-mailbox** |
...
**slice 4 분해: 4a ✅(`62e9bd6` listAllMetaIdentities, check-meta-listing 13) → 4b ✅(listEntwurfFacts 조립, `entwurf-fact-provider.ts`, quarantine+diagnostics union, check-entwurf-fact-provider 17) → 4c(MCP `entwurf_peers` 표면 + render).**
...
**F-mailbox = v2 contract mailbox-deliverability 축 (2026-06-11 발견, step 5 진입 전 BLOCKER — "나중에"가 아님):**
