...
3. **⏳ agent-config 심링크 놓기(1회, 외부 협조 — GLG-gated)** — agy mcp_config가 agent-config 심링크인 동안 adapter는 설계대로 REFUSE, doctor는 dangling FAIL(2026-07-04 실측: documented·observed 둘 다 심링크→`~/.pi/agent/git/.../entwurf/mcp/entwurf-bridge/start.sh` dangling). agent-config 쪽이 GLG 승인 하에 링크 로직(run.sh:792-797)+`antigravity/mcp_config*.json` 제거 → 이후 `install-agy-bridge`가 regular file 생성(adopt) + bare `entwurf-bridge`(②로 이제 PATH 해석됨) 기록 → doctor green. *agent-config는 이것 외 관여 안 함.* **②+③ together가 `?`를 끈다.**

**그 다음:** ④ **⑨ LIVE smoke — 작성+deterministic 검증 완료(`029941c`), 정본 LIVE는 ③+GLG "돌려" 대기.** `scripts/smoke-agy-native-push-live.ts` steps: doctor-static preflight(dangling=FAIL, ③ 시퀀싱 게이트) → probe route → register create/attach 멱등 → fire→native-push delivered → post-send re-probe(D7 partial) → owned-outcome→`native-push-no-resume-authority` → bogus-conv→`native-push-probe-indeterminate`. **격리 store(페블 승인):** `ENTWURF_META_SESSIONS_DIR` production seam으로 register+v2를 temp store에 태우고 finally 통째 정리(test-fork 0, 실 store 잔여 0 → NEXT ⑥ leftover 클래스 **구조적 종결**). **v1 스코프(진행 중 페블 통지):** `AGY_CONVERSATION_ID` 필수(GPT L1~L5 실증 경로). **canonical follow-up(정직 표기):** smoke-소유 agy launch(tmux)·`native-push-target-dead`(agy 부재 필요)·전문 content-receipt — smoke-소유 agy 생애주기 필요라 정본 LIVE와 함께. route-trace env 미도입(probe {status,route} 구조화 반환 → smoke 자기 로깅, adapter surface 최소). **deterministic 실증:** SKIP·typecheck·격리 preflight-FAIL(dangling `AGY_MCP_CONFIG`, 실 agy 무접촉). **완료판정 = 정본 LIVE green 후 `DELIVERY.md` agy verified-probe→shipped** — 승격 근거는 GPT 실-store L1~L5 + 정본 격리 smoke **함께** 인용, D7/D8 partial 정직 표기. ⑤ **push 승인**(GLG; 현재 미푸시: `af5d2ba` `da5579f` `2cea871` `029941c` + 이 docs). ⑥ 기존 잔여 meta-record `20260704T201811-071ba8`는 신규 격리 방침으로 재발 0이나 실물 1건 잔존 → **일회성 prune 별도 GLG 확인**. 잔여 선택(blocker 아님): Q14 tsconfig 이중 포함은 문서화로 갈음.
