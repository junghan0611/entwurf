...
> - **구현 (2026-06-25):** Step A(`backend-adapter.ts` = 인터페이스+claudeAdapter+fail-fast registry) + Step B(backend.ts turn loop를 어댑터 경유로 배선 — `AcpTurnDeps`에서 resolveLaunch/ensureOverlay 제거, spawnChild extraEnv, configSig nativeModelId/extra, runNewTurn에 adapter param 전달, `allCuratedModels` route/duplicate fail-fast, private resolveLaunch 삭제) 완료. **`pnpm check`/typecheck 3-config 전체 green, GPT Step A·B 승인.** turn loop/acp-client/event-mapper/session-store에 백엔드 분기 0 — 차이는 전부 `AcpBackendAdapter` 7 메서드 뒤. **LIVE smoke 4축 PASS(2026-06-25 16:52 KST):** `smoke-acp-raw-turn-live` · `smoke-acp-overlay-live` · `smoke-acp-provider-live` · `smoke-acp-session-reuse-live` EXIT 0 (`/tmp/pi-tmux-acp-adapter-live.log`). 다음 = cortex 포팅 지침 + config.ts `backend` 가드 정리. GPT future note: `extra` flat/sorted primitive, config.ts 가드 cortex 정리(§docs/acp-backend-rail.md §9).
> - **2026-06-25 저녁 (Opus 실무자 / GPT 검수자 역할 분담):** GPT가 PR#40 cortex를 현 Step A/B에 스케치→즉시 폐기(tree clean 확인). 제기한 빈틈 = "backend-specific settings(cortexConnection)를 공통 config 오염 없이 어떻게 운반하나"(§9 미해결). 처리:
>   - **✅ 구현+검증 완료 (settings.backend 가드 정리 = GPT future note #2, 미커밋):** config.ts에서 claude 하드코딩 throw 제거 → backend는 **구문 검증만**(string이면 통과, 값 화이트리스트 없음; 유효 backend는 registry 소유). `ProviderSettings.backend?`/`ResolvedAcpConfig.backend?` 추가, resolveProviderConfig surface. backend.ts 라우팅 지점에 가드 `config.backend !== adapter.backend → finishError`(**modelId = 단일 라우팅 권위**, settings.backend = 진단 가드). check-acp-config.ts assertion 4개. **typecheck 3-config + pnpm check 전체 EXIT 0.**
>   - **✅ 빌드+검증 완료 (adapterSettings opaque seam — GPT 전 항목 동의):** Q1=generic 안 감(registry heterogeneous → dispatch는 `AcpBackendAdapter<unknown>`, generic은 공유 param만 오염), Q2=flat 공통 + opaque 슬롯 1개. GPT 검수로 **threading은 (ii) 확정**(loadConfig에 routed adapter 전달 — config가 재라우팅 안 함, "routing happened twice" 회피, 코드 흐름이 modelId 단일 권위를 보여줌). 구현: `AcpBackendAdapter.resolveAdapterSettings(AcpAdapterSettingsParams{global/project/mergedBlock+paths})→unknown`(claude=undefined) · `ResolvedAcpConfig.adapterSettings:unknown`(backend.ts inspect 금지) · `readProviderSettingsFile`가 `{settings,raw}` 반환(raw 블록이 hook에 backend-specific 키 운반) · `resolveProviderConfig(params.adapter)` 배선 · `loadConfig(cwd,modelId,adapter)` · `configSignatureFields(config)`→`(config.adapterSettings)`로 좁힘. 게이트: check-acp-config에 **페이크 어댑터 plumbing assertion**(opaque 반환이 adapterSettings에 안착 / raw 블록이 hook 도달 / backend-specific 키가 공통 config에 **절대 안 뜸** / no-settings→undefined) + check-acp-session-reuse adapter 배선. **typecheck 3-config + pnpm check 전체 EXIT 0.**
>   - **✅ GPT AMBER 반영 (범용 seam 일관성):** adapterSettings를 만들었으면 **모든 backend-owned behavior seam이 도달 가능**해야 함 → `ensureOverlay()`→`ensureOverlay(AcpOverlayParams{cwd,modelId,nativeModelId,config})`(필수), `loadCarrier`에 `config` 추가(권장). `launchEnvDefaults()`는 static 유지(settings-derived env는 `ensureOverlay().envOverrides`로). 이제 resolveLaunch/buildSessionMeta/ensureOverlay/loadCarrier 전부 config(⊇adapterSettings) 접근, configSignatureFields는 opaque 직접. **재검증 typecheck 3-config + pnpm check EXIT 0.**
>   - **✅ docs/acp-backend-rail.md §10 추가** (settings.backend 가드 + adapterSettings 범용 seam 확정, GPT same-commit 권장 반영). 헤더 Status도 "claude 레일 구현 완료" 갱신.
>   - **다음 기여자 진입점:** 레일 track 완성 — 기여자는 **`adapters/cortex.ts`(또는 cortexAdapter) + registry 등록 + cortex 게이트만** 추가. backend.ts/acp-client/event-mapper/session-store/config.ts 공통층 **무수정**. cortex는 `resolveAdapterSettings`로 `cortexConnection` 파싱, `resolveLaunch`/`ensureOverlay`/`loadCarrier`/`configSignatureFields`에서 `config.adapterSettings`(또는 opaque) cast. 실제 cortex 필드 검증은 cortex adapter PR/게이트 몫.
>   - **✅ GPT 최종 GO (2026-06-25 19:20, `…341a87`):** diff 전체 + `pnpm typecheck` + targeted gates(`check-acp-overlay`/`-config`/`-session-reuse`) 직접 통과 확인, 4문항 전부 승인. 변경 의미 단일("ACP backend rail을 backend-specific settings까지 범용화") → **한 커밋** 권장. 추천 subject: `refactor(acp): generalize backend settings rail`.
>   - **✅ cortex 중립화 (GLG 지시 "cortex는 기여자 몫, 우리는 레일만"):** 실행되는 코드/fixture에서 cortex 0 — check-acp-config fixture를 `cortex/cortexConnection`→**`demobackend/demoSetting`**(가상 backend), 이번 세션 추가한 공통층 주석(backend-adapter/config)도 `cortexConnection`→일반 표현, docs §10(C) 예시도 중립화. **남은 cortex = Step A/B 기존 주석 + docs §4/§6 기여자 명세뿐**(구현 아님, 안내). 미래 우리 2번째 backend는 codex-acp(cortex 아님).
>   - **★ 기여자(hvkiefer/PR#40): 구현 선점 0 / 통합 계약 확정 (GLG 제약 "커밋푸시가 그분한테 영향 주면 안돼", GPT 자문 정밀화):** cortex 어댑터/파일/registry/게이트 **0건** → 그분이 cortex를 0부터 추가(구현 선점 없음). 우리가 만진 파일은 **레일**(공통층)뿐. **단 "영향 0"은 과장** — PR#40은 0.11 fat-bridge 위라 어차피 그대로 merge 불가이고, 이 레일은 "이 인터페이스로 옮겨달라"는 **porting 제약**을 준다(rebase conflict 아님, porting work). 그 제약이 바로 GLG가 말한 **PR 룰=표준궤**(선점 아님). 정확한 표현: *implementation ownership untouched; integration contract fixed*. push/merge 타이밍=GLG.
>   - **✅ 종결 — 커밋+푸시 완료 (2026-06-25 21:44, GLG 지시):** `d554e27` `refactor(acp): generalize backend settings rail` (9 files, +263/−50) → origin/entwurf-rename push 완료. pre-commit `pnpm check` 전체 + global identity/secret hook 통과, agenda stamp 완료. **이 레일 작업 한 사이클 종결** — cortex는 온전히 기여자(PR#40) 몫(adapter+gates만 얹음). GLG 검증은 사후. + 미해결 chore: `models.ts` getModels deprecated(L32/69) 별도 정리.
