...
현재 좌표: 0·1·2·3·4·5 완료 → **M1 complete; corrected C4 LIVE PASS** → **7(M1-b 배치 교체) checkpoint 완료** → **M2-a(`f9cea1d`)·M2-b(`07c7cbc`) 착지** → **8(M3 clean-host activation): M3-0 측정 완료 · M3-a pure leaf checkpoint 착지** → 6 보류
...
- **CURRENT: M3-b의 pinned runtime bootstrap + scoped Pi/Claude setup 경계를 측정부터 연다.** pure leaf는 아직 어느 manifest·status·setup에도 배선되지 않았다.
