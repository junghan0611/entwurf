...
What v2 provides is a **deterministic dispatch substrate** that moves the "which transport?" decision out of the fallible caller/model and into the decider, with transport-appropriate locking and an honest reject (no `✓ delivered`, no `.msg` garbage) when a target cannot receive. What it does **not** do is **fresh sibling creation** — minting a brand-new sibling from a provider/model/prompt remains outside the delivery verb (the `dormant pi → spawn-bg resume` row above resumes an *already-identified* citizen, it does not mint one). The meta-mailbox row requires an **active** self-fetch receiver; native-push requires a record-backed, probe-alive native conversation and never borrows mailbox state. The [mux launch lane](./docs/mux-launch-rail.md) has moved those three raw-tmux actions — inspect the caller's own placement, append a window inside that same session, close it on a stable handle — into a placement leaf with deterministic and real-tmux gates. That leaf is an optional, deletable one: **no production consumer imports it**, `entwurf_v2` behaves exactly as it did before, and visible launch (T1-a) and automatic delegation (T1-b) are both unimplemented. No production launcher exists yet, and mux will not become another delivery transport.
