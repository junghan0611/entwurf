...
[`entwurf_v2`](#entwurf_v2--canonical-dispatch-verb) is the canonical dispatch surface over *existing* garden citizens — live control-socket send, meta-mailbox enqueue, and native-push into a live Antigravity conversation. It starts no process on any rail: the hidden background resume that used to answer a dormant target was withdrawn under the visible-first rule, so a dormant citizen is currently unreachable and rejects honestly. The meta-record is the sole address authority (#50 C4): a record-less control socket is refused as a `record-less-socket` diagnostic, never dispatched. The v1 entwurf verbs are gone. Fresh siblings are a separate verb — `entwurf_fresh_call` opens one in the operator's own tmux session and learns its garden id from the callback it makes; the non-Claude ACP lane landed earlier — Snowflake Cortex Code became the second backend in 0.13.0.
...
- **Entwurf** (기투, projection-of-self) — sibling sessions with their own runtime boundary. Not "delegate," not "worker," not "sub-agent." Opening a visible sibling (`entwurf_fresh_call`) and live peer messaging (`entwurf_v2`) are first-class; resume is not a shipped verb — the hidden background one was withdrawn under the visible-first rule.
...
What v2 provides is a **deterministic dispatch substrate** that moves the "which transport?" decision out of the fallible caller/model and into the decider, with transport-appropriate locking and an honest reject (no `✓ delivered`, no `.msg` garbage) when a target cannot receive. It still does **not** mint siblings, and since the visible-first cut it does not relaunch one either: every row above either reaches a citizen that is already running or refuses. Fresh creation is the separate `entwurf_fresh_call` verb. It opens one fixed Pi or Claude Code sibling visibly in the caller's tmux session, returns only a synchronous launch receipt, and lets the sibling report its new address asynchronously through the sender envelope of a nonce callback. The meta-mailbox row requires an **active** self-fetch receiver; native-push requires a record-backed, probe-alive native conversation and never borrows mailbox state. The [mux launch lane](./docs/mux-launch-rail.md) owns placement, fixed-runtime launch, and this narrow fresh-call composition; delivery does not import launch, and mux is not a delivery transport.
...
**Resuming an existing garden session.** There is no resume verb. The one that existed, `entwurf_v2 intent=owned-outcome`, resumed a dormant citizen by launching a hidden, window-less background child and was withdrawn under the visible-first rule; a visible same-id resume is a separate lifecycle capability that has not landed. Re-open a session by hand with pi's own `--session` picker — the reopened session re-attaches to the same record (same garden id) at `session_start`, and it is then addressable again. The record-authoritative identity half of the old path is preserved (`resume-launch-identity.ts`, gated by `check-resume-launch-identity`) for that later lane.
