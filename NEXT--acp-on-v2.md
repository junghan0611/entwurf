...
- **S2a-1 DONE (2026-06-18, 미커밋)**: dep pin(`@agentclientprotocol/sdk@0.22.1` + `claude-agent-acp@0.39.0`) + **peer-resolution pin `@anthropic-ai/sdk@0.100.1`**(GPT 옵션A 승인 — claude-agent-sdk 0.3.156의 `>=0.93.0` peer 충족용, source 미사용). surface gate `check-acp-sdk-surface`(4 layer: pins / lock peer-resolution / wire-SDK export / no-client-use) 작성 + `pnpm check` 등록. typecheck green, 게이트 green. GPT anchor rotate: `…be0e35`→`…441eab`.
- **Next = S2a-2 (raw 1턴)**: stdio JSON-RPC ACP client + claude-agent-acp spawn → **raw 1턴 바이트 회수**(LIVE-gated · scratch cwd · `_meta.systemPrompt` 안 키우는 minimal payload). streamSimple 교체(S2c)/overlay(S2b)/session·engraving(S2d) 앞당김 금지. 상세 = §S2-scout 핀 + §구현 순서 + §홉 계획. **overlay 활성·메일박스 부재 증명은 S2b+**(backend가 돌아야 가능).
- **잠긴 방향 (굳음)**:
