...
- **Next = S2c (event mapping + streamSimple 실 backend 교체)**: raw client를 provider `streamSimple`에 연결 + `session_notification`→pi stream mapping(message_chunk→text/thought→thinking/tool_call→notice/usage→토큰·비용, Claude rawOutput=array). **여기서 backend-stub fail-loud 제거.** session·engraving·augment(S2d) 앞당김 금지. 상세 = §S2-scout 핀 + §구현 순서 + §홉 계획 + oracle F.
- **잠긴 방향 (굳음)**:
...
- **S2 컷 (≈4~5 Opus + 2~3 GPT)** — 순서가 안전장치 (**현재: S2b까지 DONE, 다음 S2c**):
  ...
  - ~~**S2b** overlay + 도구축소 + `assertExcludeToolsHonored`~~ ✅ **DONE (2026-06-18, Opus 3rd)**: overlay materializer + tool-surface pure preflight + overlay-aware live turn(child `CLAUDE_CONFIG_DIR` + `hooks:{}` + tool-narrowed `_meta`, carrier 부재). 게이트 2개(IN pnpm check) + live smoke(OUT). GPT architecture PASS + gate-hardening 반영. **미커밋(GLG commit/push 대기)**.
  - **S2c** 이벤트매핑 + `streamSimple` 실 backend 교체(stub 제거). ◀ **다음 컷**.
...
# 넘으면 안 되는 선
- **다음 coding = S2c — S0·S1·S2a·S2b DONE(GPT PASS).** §S2-scout 핀이 하드제약. S2c = `session_notification`→pi stream event mapping + `streamSimple` 실 backend 교체(**여기서 backend-stub fail-loud 제거** — provider path가 처음 열림). S2d(billing carrier/session reuse·signature/engraving/first-user augment) 앞당김 금지 — overlay+tool-narrow(S2b)가 먼저 살았음. **S2c 전(=지금)까진 fail-loud stub 유지**(`@anthropic-ai/sdk` 직접 dep는 peer-pin만, source use 금지). socket/peers/`entwurf_v2`/v2 core 손대기 금지. **v1 `entwurf`/`entwurf_send`/`entwurf_resume` 절대 부활 금지.**
