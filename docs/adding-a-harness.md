...
| `codex` | independent vendor | native-push: `thread/loaded/list` probe on the operator-owned app-server UDS, then one-shot `codex queue`; no mailbox, no receiver marker, no retry | `mcp__entwurf_bridge__entwurf_v2`; strict request metadata is the sender join | unreleased candidate: `--remote unix://<default-socket> --model <model> --dangerously-bypass-approvals-and-sandbox <prompt>`. Parser, env-boundary, setup, preflight and clause-7 composition have LANDED IN SOURCE; qualification, full floor, installed doctors and LIVE acceptance remain — so this row stays unreleased, not supported |
