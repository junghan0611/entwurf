...
- **프레이밍의 마지막 두 줄은 결과가 어디로 가는지를 말한다** — 한 줄은 도구와 target, 한 줄은 왜 그것이
  필요한지(자기 창의 출력은 호출자에게 배달되지 않는다). `[GLG 직접, 2026-09-18]` 그 문장이 없던 동안 형제가
  과제를 끝내고 답을 자기 창에만 남겼다. 도구 이름은 콜백과 **같은 backend dialect 맵**을 쓴다. 감시·강제
  송신은 없다(Hard Rule 16).
