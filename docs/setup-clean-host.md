...
| pi | optional-by-presence, `>=0.87.0 <0.88` — absent is an explicit setup SKIP, below-floor is a named FAIL | ACP provider, control sockets |
...
The supported range is `>=0.87.0 <0.88`. It is a hard minimum: installing this
...
gate, and no exact pin — unlike Node (`engines.node`), pi (`>=0.87.0 <0.88`) and Claude Code
