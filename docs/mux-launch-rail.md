...
3. **배치 소유자의 공식 보고 + 유일 키 조인** `[#116 S1에서 추가, 2026-09-14 · D5로 정정 같은 날]` —
   배치를 소유한 쪽(오늘은 herdr)의 **공식 integration이** 자기 목록에 **우리가 독립적으로 소유한 유일
   키**를 실어 보고할 때. 오늘 그 키는 `nativeSessionId`이고, Hard Rule 7이 그것을 meta-store 전체에서
   유일하게 유지한다. 두 개의 독립 사실이 하나의 유일 키 위에서 만나므로 이것은 추측이 아니다 — 위
   금지 목록(title·cwd·시각 근접)의 어느 것도 쓰지 않는다.

   **"공식"이 조건이지 장식이 아니다.** herdr는 사용자가 쓴 third-party integration의 session 보고도
   받는다. 그래서 조인은 `(source, agent, kind)` 삼중을 확인한다 — `herdr:claude`+`claude`+`id`,
   `herdr:pi`+`pi`+`path` 둘만 읽고, 한 다리라도 어긋나면 **키 없음**이다. 그리고 그 보고가 말할 수
   ...
   **두 축의 정확도가 같지 않다는 것도 적는다.** claude 축은 보고된 값이 **곧 키**다(바이트 동등).
   pi 축은 값이 세션 **파일 경로**이고, 키는 그 파일명에서 **pi 0.85.1에서 측정한 strict 변환**으로
   꺼낸다 — 유일 키 동등이 아니라 **vendor floor**다. 그 layout이 벤더 계약인지 관례인지는 미측정이고,
   그래서 이 축이 드리프트할 수 있는 유일한 자리다. 변환을 양끝에서 조여 두었으므로 드리프트는
   **놓친 조인**으로 끝나지 **틀린 조인**이 되지 않는다.

   이 항목이 **열어주지 않는 것**을 함께 적는다. 키가 유일하지 않거나, 보고가 공식 삼중을 만족하지
   않거나, 값에서 키를 위 strict 변환으로 꺼낼 수 없거나, 두 개 이상의 pane이 한 키를 주장하면 그것은
   조인이 아니다. 마지막 경우는 `ambiguous`이지 pane 하나가 아니다 — 도착 순서로 고르면 그게 곧
   추측이다.
...
| 값 | 뜻 |
|---|---|
| `unobserved` | **아무도 안 쟀다.** 이 호스트에 배치 소유자가 없거나, 읽기가 실패했거나, **읽기의 일부를 우리가 사양했다**(§7 3항의 공식 삼중을 못 만족한 보고가 하나라도 있었을 때). 사양한 그 보고가 바로 이 citizen의 것이었을 수 있으므로, 못 찾았다고 말하지 않는다. 보고된 backend가 레코드 backend와 어긋나는 모순도 여기로 온다 |
| `none` | 배치 소유자를 **읽었고**, 그 읽기가 **완전했고**, 그 목록에 이 citizen이 없다. 음성 결과를 가진 실제 측정이다 |
...
citizen은 이번 pass에서 `none`(읽기가 완전했다면) 또는 `unobserved`(무언가를 사양했다면)로 읽히고,
다음에 다시 물으면 된다.

**placement 값은 지금 관측한 프로세스의 herdr 서버에 상대적이다** `[O4]`. herdr의 pane id와 agent
name은 한 서버 scope다. 그러므로 이 값을 저장하지 않고, 다른 시점·다른 tool 결과의 값과 비교하지
않는다. 매 listing마다 다시 재는 것이 이 값이 의미를 갖는 유일한 조건이다.
...
- caller-supplied command/env/**window**-name carrier, 또는 model·cwd·session seat 외 별도 provider/settings carrier. Codex's fixed MCP `env_vars` allowlist is configuration of the operator-owned app-server boundary, not a free-form fresh-call input.
