...
**Codex native lane.** #95 is now an **unreleased candidate**, not the 2026-09-08 step-1-only state. It preserves Codex's native tools, delegation, and work context through an operator-owned app-server; it is not duplicate GPT access and remains outside ACP. Record birth, strict request-scoped identity, native-push, and visible fresh exist in the candidate. The GPT-6 review reopened parser, explicit env-boundary, setup acceptance, preflight safety, and clause-7 composition details; those amendments have since LANDED IN SOURCE. App-server tmux inheritance is only a placement mechanism: request identity has no attached-TUI seat join. First release therefore still owes real `Pi → visible Codex → visible Pi` on Oracle, with fresh Codex / app-server / outbound Pi session coordinates separately recorded and equal; a fixture first leg is insufficient. Qualification, full floor, installed doctors, and this LIVE remain release stops.
...
갖고 이슈로 승격한다. 현재 실행 계약은 #78 platform evidence (물리 맥 영수증), #95 Codex amendment/Oracle acceptance, #76 subscription rail 강제, #80 public vocabulary, #107 remote-ready routing, #108 placement wire 증거다. #72·#82·#86·#110은 CLOSED. 실행 순서는 active NEXT handoff가 진다.
...
| **Codex** | **unreleased native candidate OPEN (#95)** — amendment + deep/installed-host acceptance pending | Operator-owned app-server-backed citizen: prompt-free root `SessionStart` record birth, strict request `_meta` identity, `thread/loaded/list` probe, one-shot `codex queue`, and `entwurf_fresh_call`. The app-server's actual tmux seat reaches its MCP child through configured `env_vars`; the same explicit boundary carries custom `CODEX_HOME` and Entwurf garden/control roots. That seat is only the mechanism, not an attached-TUI join: N TUIs across sessions provide no request→TUI adjacency. Entwurf never owns the app-server lifecycle, Codex keeps its native tools/delegation/work context, and there is no ACP or resume lane. GPT-6 amendments cover parser, env boundary, setup acceptance, preflight safety, and clause-7 composition. | Oracle still owes qualification/full/install doctors plus real `Pi → visible Codex → visible Pi`; the fresh Codex, inherited app-server, and outbound Pi tmux sessions must be recorded separately and equal. A fixture/self-fetch first leg does not accept #95. |
