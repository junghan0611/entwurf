...
`v0.22.0`이 현재 패키지다 (`repair=0.12.8-repair.1` 보존). 0.22.0은 Codex caller-seat(#95 lane B/C)와 launch-directory 진단을 실었고, 그 다음 컷이 #116 herdr 레일과 #117 Codex hooks 공존을 싣는다. 0.15.0은 Copilot을 시민으로 들였고, 0.15.1은 Linux one-command setup honesty(#86)다. 0.16.0은 OMP를 출하했다. 0.19.0은 #105 ① placement였고, 0.20.0은 macOS 설치면을 **CERTIFIED (CI)** 로 실으면서 `claude-fable-5-1`을 curated 행에 더했다. Claude mailbox, pi control-socket,
...
**Codex native lane.** #95 is **supported in 0.21.0**. It preserves Codex's native tools, delegation, and work context through an operator-owned app-server; it is not duplicate GPT access and remains outside ACP. Record birth, strict request-scoped identity, native-push, and visible fresh exist. 0.21.0 shipped an explicit operator-owned tmux home named `codex` as the omitted-placement seat, and #95 D1 retired it on 2026-09-16: a Codex CALLER now opens beside its own TUI pane, matched by the `thread-id` in that pane's terminal title, and every other caller opens in its own session. Entwurf creates or supervises no app-server. The four-coordinate first-admission LIVE passed on 2026-09-12 (57 assertions) under the home topology — initial Pi outside → Codex home → outbound Pi home, with exact callbacks and addressed delivery both ways; `DELIVERY.md` owns that receipt as history. The caller-seat re-run is **accepted**: 2026-09-16, caller-seat topology 56 assertions and the caller-DIRECTORY axis 65 assertions, both exit 0 — `DELIVERY.md` and `BASELINE.md:37-38` own those receipts, and `VERIFY.md`'s release-acceptance row records them as the current contract. Exact `rust-v0.153.4` source still exposes no request→arbitrary-attached-TUI seat carrier; that wider topology is unsupported and unclaimed. Two acceptance axes are kept apart. The pre-amendment first-admission axis killed 460/460 mutants with both purity axes green and passed a frozen full floor in 506s. A later prepare P5 then exposed the model-report observer defect, and the source-observer amendment answered it: a second defect it introduced (a source path read once at correlation, frozen out when the record gained `transcriptPath` 34s later) was measured and closed with its own claim, and the amended standalone LIVE passed at **48 assertions, exit 0** with every opened window reclaimed by its own source receipt. The 48 and 57 counts are different contracts, not a regression. Current inventory is 475 mutants across 43 lanes, and that candidate then took its own full acceptance: `check:full` exit 0, and the LIVE release gate `--cut` at MUST 24/0/0 with 475/475 mutants killed and both purity axes green, its Codex leg repeating the 48-assertion acceptance. What remains is the MAKE boundary, not another measurement.

**Visible-home direction.** Upstream [openai/codex#44774](https://github.com/openai/codex/issues/44774) is where the vendor states a preference for client-side terminal/window management (multi-client steer semantics, network propagation), which is why 0.21.0 chose an operator-owned exact home plus explicit placement instead of waiting for a request→pane carrier or guessing one. #95 lane B found the carrier in what the client already presents — the `thread-id` the TUI writes into its own terminal title — so the home became unnecessary and D1 retired it. A dedicated operator-owned `claude` tmux seat is coherent with the same UX direction and is already expressible through explicit `placement.tmuxSession`; it is not yet an omitted-placement default or a new lifecycle promise. Measure and decide that policy separately instead of silently widening the 0.21.0 Codex amendment.
...
갖고 이슈로 승격한다. 현재 실행 계약은 #95 Codex caller-seat admission, #78 platform evidence (물리 맥 영수증), #76 subscription rail 강제, #80 public vocabulary, #107 remote-ready routing, #108 placement wire 증거다. #72·#82·#86·#110·#111·#112는 CLOSED. 실행 순서는 active NEXT handoff가 진다.
...
| **Codex** | **supported since 0.21.0 (#95); #95 D1 retired the fixed home 2026-09-16 and the caller-seat LIVE was ACCEPTED the same day (56 + 65 assertions)** | Operator-owned app-server-backed citizen: trust-gated user-scope `SessionStart` birth in `$CODEX_HOME/hooks.json` (no root; the operator answers the vendor prompt once), strict request `_meta` identity, `thread/loaded/list` probe, one-shot `codex queue`, and `entwurf_fresh_call`. Supported placement follows the CALLER: a Codex citizen opens beside its own TUI pane via the `thread-id` in that pane's title (0/2+ reject). Entwurf never owns the app-server lifecycle, and there is no ACP or resume lane. | Two axes, kept apart, both under the retired home topology: pre-amendment first-admission four-coordinate `Pi outside → Codex home → Pi home`, exact callbacks/addressed delivery — green, 57 assertions, qualification 460/460, frozen full floor green; observer-amended standalone LIVE — green, 48 assertions, source audit 3/3 both sides, windows reclaimed by source receipt only (`DELIVERY.md` owns both). Current acceptance: 475/475 qualification, release gate MUST 24/0/0, `cut: OK`. Arbitrary attached-TUI seat inference remains unsupported. |
