...
## 현재 — 0.20.1 shipped; 0.21.0에 #95 Codex home-topology 준비
...
| **Codex** | **supported in 0.21.0 (#95), explicit-home acceptance green** | Operator-owned app-server-backed citizen: trust-gated user-scope `SessionStart` birth in `$CODEX_HOME/hooks.json` (no root; the operator answers the vendor prompt once), strict request `_meta` identity, `thread/loaded/list` probe, one-shot `codex queue`, and `entwurf_fresh_call`. Supported placement is the existing exact `codex` tmux home containing app-server + supported TUIs; omitted Codex fresh selects it. Entwurf never owns that lifecycle, and there is no ACP or resume lane. | Two axes, kept apart: pre-amendment first-admission four-coordinate `Pi outside → Codex home → Pi home`, exact callbacks/addressed delivery — green, 57 assertions, qualification 460/460, frozen full floor green; observer-amended standalone LIVE — green, 48 assertions, source audit 3/3 both sides, windows reclaimed by source receipt only (`DELIVERY.md` owns both). Current acceptance: 475/475 qualification, release gate MUST 24/0/0, `cut: OK`. Arbitrary attached-TUI seat inference remains unsupported. |
