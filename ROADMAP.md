...
| pi 0.80.3 fence | `pnpm check` + release-gate MUST |
...
- **`/gnew` T3 backend axis** — 현재 claude-sonnet-5만 측정. codex/gemini로 확장.
...
- **pi 0.80 public export:** `hasProjectTrustInputs`/`ProjectTrustStore`/`getAgentDir`/`VERSION` 모두 index
  public export → TS 직접 import(재구현 불필요). floor = **0.80.3** (`>=0.80.3 <0.81`, next-minor 상한).
