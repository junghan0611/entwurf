...
# NOW — H1 홉 1 진행 + V2 검수 (2026-09-19 14:0x)

- **좌표:** main `37b81e7` = v0.23.0 + 2. 브랜치 HEAD `1991090`(V2 끝). `pnpm check` 50→52s.
- **진행 중:** Opus(garden `20260919T133137-746e24`) = H1 홉 1 `smoke-herdr-raw-install-live`(clean node:24 컨테이너, 공개 main `37b81e7` `--ref`, `--yes`, npm identity+sha512, `{pi}`→`{pi,claude-code}`; 첫 측정 = 0.9.1 서버 없는 install). terra(읽기 전용) = V2 검수 `main..1991090` → #119 코멘트.
- **다음 한 수:** terra finding을 Blocker/Defect/Observation으로 받아 amendment 1번들을 Opus에게(H1 홉 1 뒤). H1 홉 1 영수증 (a) → #118. 그 뒤 H1 홉 2(`entwurf pi`) → 홉 3 → V3(mutant 0 순수 8개부터: decider·runner·socket-discovery·matrix·facts·send-fallback·release·resume-args; v2-production/v2-contract는 cross-lane이라 첫 슬라이스 제외).
